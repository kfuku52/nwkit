...
class CalibratedSearch:
    def __init__(
        ...
        exact_count = np.count_nonzero(self.variances == 0)
        self.singular_error_boundary = self.known_error and exact_count > 1
        if self.singular_error_boundary and max_shifts >= exact_count - 1:
            raise ValueError(
                "Mixed zero/positive observation errors allow a candidate to saturate the exact observations; "
                "the Gaussian likelihood is unbounded as process variance tends to zero. "
                "Review the exact-zero error assumptions or reduce --max-shifts."
            )
        self.grid = (
    ...
    def profile(self, Z):
        ...
        if self.known_error:
            ...
            if self.singular_error_boundary and np.any(
                selected_variances == self.variance_grid[1]
            ):
                raise ValueError(
                    "Process variance reached the singular lower grid boundary; a finite regular fit "
                    "has not been established. Review exact-zero observation errors."
                )
        return best, at
    ...
    def fit(self, values, seed=1, replicates=199, level=0.05):
        ...
        for stage, family in enumerate(self.families):
            ...
            if stage == 0 and not self.known_error:
                p, calibration = self._null_probability(
                    observed, rng, replicates, level
                )
            else:
                p = self._calibrated_probability(
                    mean, L, scale, family, observed, rng, replicates
                )
                calibration = {"p_value_kind": "plugin", "p_value_lower_bound": p}
            tests.append(
                {
                    "stage": stage,
        ...
        return {
            ...
            "null_calibration": "plugin_known_error"
            if self.known_error
            else "alpha_grid_envelope",
            "seed": seed,
        }
    ...
    def _calibrated_probability(
        ...
        return self._probability_from_noise(mean, L, scale, family, observed, noise)

    def _probability_from_noise(self, mean, L, scale, family, observed, noise):
        replicates = noise.shape[1]
        exceedances = 0
    ...
    def _null_probability(self, observed, rng, replicates, level):
        # Under the no-error null, the improvement statistic is invariant to
        # baseline and process scale. Only alpha is an unknown nuisance.
        # A rejection must survive every alpha in the fitted grid.
        noise = rng.normal(size=(self.d, replicates))
        maximum = 0.0
        evaluated = []
        for index, item in enumerate(self.cache):
            p = self._probability_from_noise(
                np.zeros(self.d), item[2], 1.0, self.families[0], observed, noise
            )
            maximum = max(maximum, p)
            alpha = self.grid[item[0]]
            evaluated.append(
                {
            ...
            if maximum > level and index < len(self.cache) - 1:
                # Acceptance is already certain. Report a conservative upper
                # bound, never pass a partial maximum off as the supremum p.
                return 1.0, {
                    "p_value_kind": "conservative_upper_bound",
                    "p_value_lower_bound": maximum,
                    "null_alpha_evaluations": evaluated,
                }
        return maximum, {

...
def compile_species_regex(species_regex=None):
    regex_text = (
        DEFAULT_SPECIES_REGEX if (species_regex is None) else str(species_regex).strip()
    )
    if regex_text == "":
        return None
    try:
        ...
        raise ValueError(
            "--species-regex is not a valid regular expression: {}".format(exc)
        ) from exc
...
def read_species_map_tsv(species_map_tsv):
    ...
    try:
        ...
        if ("species_label" not in fieldnames) and ("taxonomy_query" not in fieldnames):
            raise ValueError(
                '--species-map-tsv must contain "species_label" and/or "taxonomy_query" columns.'
            )
        overrides = dict()
        for row in reader:
            ...
            if leaf_name.strip() == "":
                raise ValueError(
                    '--species-map-tsv contains an empty "leaf_name" value.'
                )
            if leaf_name in overrides:
                raise ValueError(
                    'Duplicate "leaf_name" entries are not supported in --species-map-tsv: {}'.format(
                        leaf_name
                    )
                )
...
class SpeciesParser:
    def __init__(self, preset=None, species_regex=None, species_map_tsv=None):
        ...
        if self.preset not in SUPPORTED_SPECIES_PARSERS:
            raise ValueError(
                "--species-parser must be one of: {}".format(
                    ", ".join(SUPPORTED_SPECIES_PARSERS)
                )
            )
    ...
    def _parse_legacy(self, label):
        species_label = _extract_species_label_with_regex(
            label=label, species_pattern=self.species_pattern
        )
    ...
    def _parse_taxonomic(self, label):
        ...
        if normalized_label is None:
            return ParsedSpecies()
        tokens = [
            _canonical_taxonomic_token(token)
            for token in normalized_label.split("_")
            if token != ""
        ]
        if len(tokens) < 2:
            return ParsedSpecies()
        ...
        if (len(tokens) >= 3) and (
            tokens[2].lower() in TAXONOMIC_SPECIES_SUFFIX_TOKENS
        ):
            return ParsedSpecies(
                species_label="_".join(tokens[:3]),
        ...
        if (len(tokens) >= 4) and (
            tokens[2].lower() in set(TAXONOMIC_RANK_ALIASES.values())
        ):
            return ParsedSpecies(
    ...
    def _apply_overrides(self, label, parsed_species):
        ...
        species_label = (
            override.species_label
            if override.species_label is not None
            else parsed_species.species_label
        )
        taxonomy_query = (
            override.taxonomy_query
            if override.taxonomy_query is not None
            else parsed_species.taxonomy_query
        )
...
def get_species_parser(
    args=None, species_parser=None, species_regex=None, species_map_tsv=None
):
    if (
        (args is not None)
        and (species_parser is None)
        and (species_regex is None)
        and (species_map_tsv is None)
    ):
        cache_key = (
        ...
        args._nwkit_species_parser_cache_key = cache_key
        args._nwkit_species_parser_cache = parser
        return parser
    return SpeciesParser(
        preset=species_parser
        if (species_parser is not None)
        else getattr(args, "species_parser", DEFAULT_SPECIES_PARSER),
        species_regex=species_regex
        if (species_regex is not None)
        else getattr(args, "species_regex", DEFAULT_SPECIES_REGEX),
        species_map_tsv=species_map_tsv
        if (species_map_tsv is not None)
        else getattr(args, "species_map_tsv", None),
    )


def extract_parsed_species(
    label, args=None, species_parser=None, species_regex=None, species_map_tsv=None
):

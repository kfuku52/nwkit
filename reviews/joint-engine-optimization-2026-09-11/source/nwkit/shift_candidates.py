...
def enumerate_candidates(tree, max_shifts=2, limit=5000, *, convergence=True):
    ...
    if not heights or not all(math.isfinite(h) and h > 0 for h in heights):
        raise ValueError("Joint reference requires finite positive tree height")
    ...
    for size in range(max_shifts + 1):
        for selected in itertools.combinations(branches, size):
            ...
            for groups in sorted(
                ...
                if not convergence and len(groups) != size + 1:
                    continue

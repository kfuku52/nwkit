...
class VectorTreeWhitening:
    ...
    @classmethod
    def build(cls, compiled, mask, slopes, innovations, errors, *, root_covariance):
        ...
        leaf_rows = {node: row for row, node in enumerate(compiled.leaf_indices)}
        ...
        for node in compiled.postorder:
            if node in leaf_rows:
                ...
                step = _Step(
                    node, (), positions[row, observed], None, len(observed), factor
                )
            else:
                children = tuple(i for i in compiled.children[node] if i in messages)
                if not children:
                    continue

...
def _projected_gradient(function, x, bounds):
...
def _general_fit(data, layout, options, fixed_alpha, fixed_process, fixed_noise):
    ...
    def decode(x):
        ...
        noise = x[offset : offset + p] ** 2 if noise_count else fixed_noise
    ...
    def evaluate(x):
        ...
        return evaluate_joint(
            data, layout, alpha, covariance, noise, root_model=options.root_model
        )
    ...
    for index in range(options.optimizer_starts):
        ...
        if noise_count:
            x.extend(np.sqrt(np.diag(initial) * noise_fraction))
        x = np.asarray(x)
        if len(x):
            result = minimize(
                objective,
            ...
            score = (
                _projected_gradient(objective, result.x, bounds)
        ...
        try:
            ...
            if separable:
                ...
                fit, diagnostic = _general_fit(
                    data, layout, local_options, candidate_alpha, process, noise
                )
            records.append(
                {
                    "alpha_mode": mode,

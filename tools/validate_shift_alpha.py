...
def write_table(path, rows, fields=None):
    with path.open("w", newline="") as handle:
        writer = csv.DictWriter(
            handle, fieldnames=fields or list(rows[0]), delimiter="\t"
        )
        writer.writeheader()
    ...
    write_table(
        ...
        [
            ...
            for name, value in zip(
                truth["tip_names"], truth["observations"], strict=True
            )
        ],
    )
...
def main():
    ...
    snapshot.mkdir()
    for name in (
        "shift_alpha_design.py",
        "shift_alpha_backend.R",
        "validate_shift_alpha.py",
    ...
    (root / "installed-backend-sha256.json").write_text(
    ...
    with ThreadPoolExecutor(max_workers=specification["workers"]) as pool:
        futures = [
            pool.submit(run_case, folder, args.rscript, backend, environment)

...
def generate_trees(directory, case, seed):
    ...
    for node in gene.traverse():
        if node is gene:
            continue
        shift = (
            case.get("copy_ratio", 1.0)
            if all(tip.name.endswith("_2") for tip in node.leaves())
            else 1.0
        )
        rate = 0.01 * shift * math.exp(rng.normal(0, case["rate_sd"]))
        rates.append(
            dict(
                clade=sorted(t.name for t in node.leaves()),
                duration=node.dist,
                rate=rate,
            )
        )
    ...
    save_json(
        ...
        dict(age=truth, target="D", rates=rates, seed=seed, case=case),
    )
    return truth
...
def evaluate(case, family, options):
    ...
    try:
        ...
        if not sequences or any(
            len(seq) != 3 * case["codons"] for seq in sequences.values()
        ):
            raise ValueError("AliSim did not return the requested number of codons")
        if any(
            seq[i : i + 3] in {"TAA", "TAG", "TGA"}
        ...
        for method in ["none", "profile"]:
            ...
            if target is None:
                ...
                break
            estimate = finite_number(target["estimated_age"])
            row[method + "_estimate"] = estimate
            if estimate is None:
                row["status"] = method + "-nonfinite-estimate"
                break
            # Publish only a valid point; an invalid profile result must not
            # erase a completed point-only fit or crash the remaining families.
            row["estimate"] = estimate
            row["point_success"] = True
            row["bias"] = estimate - truth
            if method == "profile":
                lower, upper = (
                    finite_number(target[k])
                    for k in ["interval_lower", "interval_upper"]
                )
                row.update(
...
def summarize(rows):
    ...
    result = dict(
        families=n,
        point_successes=len(points),
        intervals_returned=len(available),
        truth_covered=covered,
        coverage_among_returned=covered / len(available) if available else None,
        correct_return_fraction=covered / n,
        availability=len(available) / n,
        bias=float(np.mean([r["bias"] for r in points])) if points else None,
        rmse=float(np.sqrt(np.mean([r["bias"] ** 2 for r in points])))
        if points
        else None,
        median_width=float(np.median([r["width"] for r in available]))
        if available
        else None,
        statuses=dict(Counter(r["status"] for r in rows)),
        interval_statuses=dict(
            Counter(r.get("interval_status", r["status"]) for r in rows)
        ),
        estimators=dict(
            Counter(
                r.get("profile_estimator", r.get("none_estimator", "unavailable"))
                for r in rows
            )
        ),
    )
    for label, numerator, denominator in [
        ("coverage", covered, len(available)),
        ("availability", len(available), n),
        ("correct_return", covered, n),
    ]:
        ci = (
            binomtest(numerator, denominator).proportion_ci(method="wilson")
            if denominator
            else None
        )
...
def main():
    ...
    metadata = dict(
        ...
        protocol_sha256=digest(options.protocol),
        runner_sha256=digest(__file__),
        generator_sha256={
            p.name: digest(p)
            for p in [
                ROOT / "tools/radte_benchmark_cases.py",
                ROOT / "tools/radte_interval_simulation.py",
            ]
        },
        source_sha256={
            p.name: digest(p) for p in sorted((ROOT / "nwkit").glob("*.py"))
        },
        iqtree_sha256=digest(options.iqtree),
        iqtree_version=subprocess.check_output(
            [options.iqtree, "--version"], text=True
        ),
        threads={k: v for k, v in os.environ.items() if k.endswith("NUM_THREADS")},
        workers=options.workers,
        timeout_per_stage=options.timeout,
    )
    save_json(options.output / "metadata.json", metadata)
    rows = []
    with (
        ThreadPoolExecutor(max_workers=options.workers) as pool,
        (options.output / "cases.jsonl").open("w") as handle,
    ):
        futures = [
            pool.submit(evaluate, c, i, options)
            for c in cases
            for i in range(c["families"])
        ]
        for future in as_completed(futures):
            ...
            handle.flush()
            print(
                f"{len(rows)}/{len(futures)} {row['case']} {row['status']}", flush=True
            )
    save_json(
        options.output / "summary.json",
        {
            c["name"]: summarize([r for r in rows if r["case"] == c["name"]])
            for c in cases
        },
    )
    metadata["changed_source_files_during_run"] = [
        name
        for name, value in metadata["source_sha256"].items()
        if digest(ROOT / "nwkit" / name) != value
    ]
    metadata["runner_changed_during_run"] = (
        digest(__file__) != metadata["runner_sha256"]
    )
    save_json(options.output / "metadata.json", metadata)

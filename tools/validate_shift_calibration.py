...
def main():
    ...
    parser.add_argument("--focus", choices=["all", "weak-null"], default="all")
    args = parser.parse_args()
    args.output.mkdir(parents=True, exist_ok=False)
    design = protocol(args.replicates, args.extension_replicates, seed=args.seed)
    if args.focus == "weak-null":
        design["cells"] = [
            cell
            for cell in design["cells"]
            if cell["family"] == "weak_pull" and cell["scenario"] == "null"
        ]
        if not design["cells"]:
            raise ValueError("Weak-null focus requires positive extension replicates")
    design["focus"] = args.focus
    for key in (
        "floors",
        "criteria",
        "methods",
    ...
    design.update(
        ...
        null_calibration_no_error="alpha_grid_envelope",
        null_calibration_known_error="plugin",
    ...
    snapshot.mkdir()
    for source in design["source_sha256"]:
    ...
    lines = [
        ...
        "",
        f"Seed {args.seed}; B=199; stagewise nominal level 0.05. Plug-in tests and finite nuisance grids do not guarantee uniform 5% error.",
        "",

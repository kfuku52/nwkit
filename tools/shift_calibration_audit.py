...
def audit_record(row, design, search, *, replay_bootstrap=False):
    ...
    if replay_bootstrap:
        ...
        if probabilities(replay["tests"]) != probabilities(tests):
            raise ValueError(
        ...
        for key in (
            ...
            if not np.allclose(
                np.asarray(fit[key], dtype=float),
                np.asarray(replay[key], dtype=float),
                rtol=1e-9,
                atol=1e-10,
                equal_nan=True,
            ):
                raise ValueError(f"Saved {key} disagrees with seeded fit replay")
...
def _audit_case(payload):
    row, design, replay_bootstrap = payload
    c = row["case"]
    key = row["tree"], c["standard_error"], design["max_shifts"]
    if key not in _AUDIT_ENGINES:
        _AUDIT_ENGINES[key] = CalibratedSearch(
            read_tree(row["tree"], "auto", True, quiet=True),
            max_shifts=design["max_shifts"],
            variances=np.full(c["tips"], c["standard_error"] ** 2),
        )
    return audit_record(
        row, design, _AUDIT_ENGINES[key], replay_bootstrap=replay_bootstrap
    )


def audit_records(rows, design, summary, *, replay_bootstrap=False, workers=1):
    payloads = [(row, design, replay_bootstrap) for row in rows]
    if workers == 1:
        results = list(map(_audit_case, payloads))
    else:
        results = []
        with ProcessPoolExecutor(max_workers=workers) as executor:
            for start in range(0, len(payloads), 16):
                results.extend(executor.map(_audit_case, payloads[start : start + 16]))
                print(f"Evidence replay: {len(results)}/{len(rows)}", flush=True)

...
def _values_equal(target_value, source_value):
    if isinstance(target_value, dict) and isinstance(source_value, dict):
        return target_value.keys() == source_value.keys() and all(
            _values_equal(target_value[key], source_value[key]) for key in target_value
        )
    if (
...
def _root_edge_report_values(nodes, properties):
    ...
    node_properties = [
        _property_values(_property_json(node, properties)) for node in nodes
    ]
    property_values = {}
    for _, output_prop in properties:
        values = [
            values.get(output_prop, _MISSING_EDGE_VALUE) for values in node_properties
        ]
...
def _rows_have_differences(rows, comparison="rooted"):
    return any(
        (row["status"] in DIFFERENCE_STATUSES or _row_has_value_difference(row))
        and not (
            comparison == "unrooted"
...
def _row_for_nodes(
    ...
    return {
        ...
        "target_branch_id": target_ids.get(id(target_node), "")
        if target_node is not None
        else "",
        "source_branch_id": source_ids.get(id(source_node), "")
...
def _summary_row(
    comparison, status, reason, shared_key="", target_taxa="", source_taxa=""
):
...
def _rooted_rows(
    target, source, mapping, target_class, properties, target_ids, source_ids
):
    rows = list()
...
def compare_trees(
    ...
    target_ids = {
        id(node): branch_id for node, branch_id in assign_branch_ids(target).items()
    }
    source_ids = {
        id(node): branch_id for node, branch_id in assign_branch_ids(source).items()
    }
    rows = [
        _summary_row(
            comparison="leaf_set",
            status="same"
            if not mapping.target_only_taxa and not mapping.source_only_taxa
            else "different",
    ...
    if comparison == "rooted":
        ...
        rows.extend(
            _unrooted_rows(
                ...
                target_ids=target_ids,
                source_ids=source_ids,
                same_leaf_set=not mapping.target_only_taxa
                and not mapping.source_only_taxa,
            )
        )
    else:
...
def diff_main(args):
    ...
    if getattr(args, "fail_on_difference", False) and _rows_have_differences(
        rows, comparison=getattr(args, "comparison", "rooted")
    ):

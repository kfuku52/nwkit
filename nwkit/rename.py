...
def read_name_tsv(path):
    ...
    try:
        ...
        if not required.issubset(fieldnames):
            raise ValueError(
                "--name-tsv must contain 'old_name' and 'new_name' columns."
            )
        mapping = dict()
        for row in reader:
            ...
            if old_name in mapping:
                raise ValueError(
                    "Duplicated 'old_name' entries are not supported in --name-tsv: {}".format(
                        old_name
                    )
                )
...
def _rename_by_mapping(tree, args):
    ...
    target_name_to_nodes: dict[str, list[Any]] = {}
...
def rename_main(args):
    ...
    if has_name_tsv == has_pattern:
        raise ValueError(
            "Specify exactly one of '--name-tsv' or '--pattern/--replacement'."
        )
    ...
    if args.check_leaf_uniqueness:
        validate_unique_named_leaves(
            tree, option_name="--infile", context=" after 'rename'"
        )
    sys.stderr.write(
        "Renamed {} target node(s) using {}.\n".format(renamed_count, mapping_label)
    )

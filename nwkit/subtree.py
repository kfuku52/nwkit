...
def subtree_main(args):
    ...
    seed_leaf_nodes = [ l for l in tree.leaves() if l.name in seed_leaf_names ]
    if len(seed_leaf_nodes)==1:
        mrca = seed_leaf_nodes[0]
    else:
        mrca = tree.common_ancestor([n.name for n in seed_leaf_nodes])
    if args.orthogroup:
        tree = annotate_scientific_names(tree)
        tree = annotate_duplication_confidence_scores(tree)
        all_sci_names = list(set([ l.props.get('sci_name') for l in tree.leaves() ]))
        num_all_species = len(all_sci_names)
        sys.stderr.write('Number of species in input tree: {:,}\n'.format(num_all_species))
        mrca_sci_names = list(set([ l.props.get('sci_name') for l in mrca.leaves() ]))
        num_species_last_updated = len(mrca_sci_names)
        node_species_last_updated = mrca
        for i,current_node in enumerate(mrca.ancestors()):
            if current_node.is_root:
                break
            if num_species_last_updated == num_all_species:
                break
            current_sci_names = list(set([ l.props.get('sci_name') for l in current_node.leaves() ]))
        ...
        num_species_subtree = len(list(set([ l.props.get('sci_name') for l in subtree.leaves() ])))

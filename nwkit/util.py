...
from nwkit.file_paths import (
    normalized_missing_path_key as normalized_missing_path_key,
)
from nwkit.file_paths import (
    paths_identify_same_file as paths_identify_same_file,
)
from nwkit.file_paths import (
    validate_distinct_output_paths as validate_distinct_output_paths,
)
from nwkit.file_paths import (
...
def is_missing_table_value(value, missing_values=None):
    ...
    try:
        ...
        if type(value).__name__ in {"NAType", "NaTType"}:
            return True
    markers = (
        parse_table_missing_values(missing_values)
        if not isinstance(missing_values, set)
        else missing_values
    )
...
def read_tip_table(
    ...
    *,
    ...
    if missing_columns:
        raise ValueError(
            "Missing required column(s) in '{}': {}".format(
                option_name, ", ".join(missing_columns)
            )
        )
    dataframe = dataframe.copy()
    dataframe["leaf_name"] = [
        str(leaf_name) for leaf_name in dataframe["leaf_name"].tolist()
    ]
    if any(leaf_name.strip() == "" for leaf_name in dataframe["leaf_name"].tolist()):
        raise ValueError(
            "Column 'leaf_name' in '{}' must not contain empty values.".format(
                option_name
            )
        )
    duplicated = (
        dataframe.loc[dataframe["leaf_name"].duplicated(keep=False), "leaf_name"]
        .unique()
        .tolist()
    )
    ...
    if tree_leaf_names is not None:
        ...
        if unmatched == "warn":
            if table_only:
                sys.stderr.write(
                    "Rows in {} not found in tree: {}\n".format(
                        option_name, " ".join(table_only)
                    )
                )
            if tree_only:
                sys.stderr.write(
                    "Tree tips not found in {}: {}\n".format(
                        option_name, " ".join(tree_only)
                    )
                )
...
def resolve_download_dir(args=None):
    ...
    if normalized.lower() == "inferred":
        ...
        if outfile not in ["", None, "-"]:
            ...
            inferred_base = os.path.realpath(
                raw_out_dir if raw_out_dir not in ["", None] else os.getcwd()
            )
...
def _find_existing_ete_taxonomy_assets(exclude_dbfile=None):
    ...
    for cache_dir in COMMON_ETE_CACHE_DIRS:
        ...
        if not os.path.isfile(dbfile):
            continue
        if not _validate_ete_taxonomy_db(
            dbfile, require_traverse=True, full_check=True
        ):
            continue
        if (
            excluded_realpath is not None
            and os.path.realpath(dbfile) == excluded_realpath
        ):
            continue
...
def _seed_ete_taxonomy_assets(target_dbfile, source_assets):
    ...
    if source_taxdump_file is not None:
        if _validate_ete_taxdump(source_taxdump_file):
            _atomic_copy_file(
                source_taxdump_file, os.path.join(target_dir, "taxdump.tar.gz")
            )
...
def _quoted_node_name_flags(newick_text):
    ...
    expected_name_kind: str | None = "leaf"
    comment_depth = 0
    i = 0
    while i < len(text):
        char = text[i]
...
def _read_tree_auto(infile):
    ...
    preferred_formats = (
        (1, 0) if quoted_internal_names or other_internal_names else (0, 1)
    )
...
def _assert_lock_path_is_regular_file(lock_path, lock_label="Lock"):
    ...
    if not stat.S_ISREG(lock_stat.st_mode):
        raise IsADirectoryError(
            "{} path exists but is not a file: {}".format(lock_label, lock_path)
        )
...
def acquire_exclusive_lock(
    ...
    if lock_dir != "":
        if os.path.exists(lock_dir) and (not os.path.isdir(lock_dir)):
            raise NotADirectoryError(
                "Lock parent path exists but is not a directory: {}".format(lock_dir)
            )
    ...
    while True:
        ...
        if not has_reported_wait:
            sys.stderr.write(
                "Another process holds {}. Waiting for lock release: {}\n".format(
                    lock_label, lock_path
                )
            )
...
def _validate_ete_taxdump(taxdump_file):
    ...
    try:
        if (
            os.path.getsize(taxdump_file) <= 0
            or os.path.getsize(taxdump_file) > ETE_TAXDUMP_MAX_BYTES
        ):
            return False
...
def _validate_ete_taxonomy_db(dbfile, require_traverse=False, full_check=False):
    ...
    try:
        ...
        try:
            if full_check and connection.execute("PRAGMA quick_check").fetchone() != (
                "ok",
            ):
                return False
            table_names = {
...
def _atomic_copy_file(source, target):
    ...
    fd, temporary = tempfile.mkstemp(
        prefix=".{}.".format(os.path.basename(target)), dir=target_dir
    )
    os.close(fd)
    try:
...
def _bounded_response_bytes(response, maximum_bytes, label):
    ...
    if content_length is not None:
        try:
            ...
            raise ValueError(
                "{} response has an invalid Content-Length.".format(label)
            ) from exc
        if declared_length < 0 or declared_length > maximum_bytes:
            raise ValueError(
                "{} response exceeds the download size limit.".format(label)
            )
    chunks = list()
    downloaded = 0
...
def _download_ete_taxdump(taxdump_file):
    ...
    try:
        ...
        if _validate_ete_taxdump(taxdump_file):
            ...
            local_md5 = hashlib.md5(usedforsecurity=False)
            ...
            if local_md5.hexdigest() == expected_md5:
                return False
        fd, temporary = tempfile.mkstemp(
            prefix=".taxdump.", suffix=".tar.gz", dir=target_dir
        )
        os.close(fd)
        try:
            downloaded = 0
...
def _build_ete_taxonomy_database(dbfile, taxdump_file):
    ...
    with tempfile.TemporaryDirectory(
        prefix=".taxonomy-build-", dir=target_dir
    ) as work_dir:
        ...
        if not _validate_ete_taxonomy_db(
            temporary_db, require_traverse=True, full_check=True
        ):
...
def _taxonomy_cache_max_age_seconds(args):
    value = getattr(
        args, "taxonomy_cache_max_age_days", ETE_TAXONOMY_DEFAULT_MAX_AGE_DAYS
    )
    try:
        days = float(value)
    except (TypeError, ValueError) as exc:
...
def _taxonomy_cache_is_stale(dbfile, args):
    if bool(getattr(args, "refresh_taxonomy_cache", False)):
        return True
    return time.time() - os.path.getmtime(dbfile) > _taxonomy_cache_max_age_seconds(
        args
    )
...
def get_ete_ncbitaxa(args=None):
    ...
    with acquire_exclusive_lock(lock_path=lock_path, lock_label="ETE4 taxonomy DB"):
        ...
        if (
            ...
            _seed_ete_taxonomy_assets(
                target_dbfile=dbfile, source_assets=existing_assets
            )
            return ete4.NCBITaxa(dbfile=dbfile, update=False)
...
def read_tree(
    ...
    *,
    ...
    if (not quoted_node_names) and _contains_quoted_node_names(infile):
        raise ValueError(
            "Quoted node names were found in the input tree. Re-run with --quoted-node-names yes."
        )
    ...
    if not allow_non_finite:
        for node in tree.traverse():
            ...
            if node.support is not None and not math.isfinite(float(node.support)):
...
def write_tree(tree, args, format, quiet=False, props=None, name_quote=None):
    ...
    if tree_str.endswith(":;"):
        tree_str = tree_str[:-2] + ";"
    if node_name_dict:
        placeholder_pattern = _compile_node_placeholder_pattern(placeholder_prefix)
...
def split_newick_stream(newick_text):
    ...
    while i < len(text):
        ...
        if quote_char is not None:
            if char == quote_char and (i + 1 < len(text) and text[i + 1] == quote_char):
    ...
    for node in tree.traverse(strategy="postorder"):
        ...
        for child in node.get_children():
            if child.dist is None:
                raise ValueError(
                    "Tree must contain all branch lengths to test ultrametricity."
                )
...
def read_seqs(seqfile, seqformat, quiet):
    if str(seqformat).lower() != "fasta":
        raise ValueError(
            "Unsupported sequence format '{}'. Only 'fasta' is supported.".format(
                seqformat
            )
        )
    if seqfile == "-":
...
def label2sciname(
    ...
    return scinames[0] if is_str_input else scinames


def extract_species_label(
    label,
    species_regex=None,
    out_delim="_",
    args=None,
...
def get_species_group_records(
    ...
    leaf_name_to_species_label: dict[Any, str] = {}
    species_label_to_leaf_names: defaultdict[str, list[Any]] = defaultdict(list)
    species_label_to_taxonomy_query: dict[str, str | None] = {}
    unresolved_leaf_names: list[str] = []
    ...
    for species_label in species_label_to_leaf_names.keys():
        if species_label_to_taxonomy_query.get(species_label) is None:
            species_label_to_taxonomy_query[species_label] = species_label.replace(
                "_", " "
            )
    return (
...
def get_monophyletic_species_groups(
    tree,
    option_name="--infile",
    context="",
    args=None,
    ...
    leaf_name_to_species_label, species_label_to_leaf_names, _ = (
    ...
    for node in tree.traverse(strategy="postorder"):
        if node.is_leaf:
            if node.name not in leaf_name_to_bit:
                raise ValueError(
                    "Leaf label not found in reference mapping: {}".format(node.name)
                )
...
def validate_unique_named_leaves(tree, option_name, context=""):
    ...
    if len(leaf_names) != len(set(leaf_names)):
        raise ValueError(
            "Duplicated leaf labels are not supported in '{}'{}.".format(
                option_name, context
            )
        )
    has_missing = any(
...
def annotate_scientific_names(
    tree, species_regex=None, args=None, species_parser=None, species_map_tsv=None
):
...
def is_all_leaf_names_identical(tree1, tree2, verbose=False):
    ...
    if verbose:
        if not is_all_leaf_names_identical:
            ...
            unmatched_names = [
                str(name)
                for name in sorted(all_names, key=lambda x: str(x))
                if counts1.get(name, 0) != counts2.get(name, 0)
            ]
            sys.stderr.write(
                "Unmatched leaf labels: {}\n".format(" ".join(unmatched_names))
            )

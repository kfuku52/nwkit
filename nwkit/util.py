...
def normalize_phylogenetic_tree_text(tree_text, collection=False):
    ...
    if main_output_tree is not None:
        ...
        if not nexus_trees:
            raise ValueError('NEXUS input did not contain a complete TREE or UTREE statement.')
        ...
        if (
            ...
            and _PAML_TREEFILE_HEADER_PATTERN.fullmatch(lines[first_content_index])
        ):
            ...
            text = '\n'.join(
                trees[:expected_tree_count] if collection else trees[:1]
            )
...
def read_tree(infile, format, quoted_node_names, quiet=False, allow_non_finite=False):
    ...
    infile = normalize_phylogenetic_tree_text(read_input_text(infile), collection=False).strip()
    if infile == '':
        raise Exception('Failed to parse the input tree.')
...
def iter_tree_strings(infile):
    ...
    if os.path.isfile(infile):
        with open(infile) as handle:
            ...
            is_paml_container = (
                first_content_line.upper().startswith('#NEXUS')
...
def inspect_tree_text(newick_text, format='auto', quoted_node_names=True):
    text = normalize_phylogenetic_tree_text(
        str(newick_text),
        collection=False,
    ).strip()
    if text == '':

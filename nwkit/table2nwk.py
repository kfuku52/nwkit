...
def _parse_int(value, column_name):
    try:
        ...
        raise ValueError(
            "Column '{}' must contain integers.".format(column_name)
        ) from exc
...
def _parse_optional_float(value, column_name):
    ...
    try:
        ...
        raise ValueError(
            "Column '{}' must contain numeric values when present.".format(column_name)
        ) from exc
    if not math.isfinite(number):
        raise ValueError(
            "Column '{}' must contain finite numeric values when present.".format(
                column_name
            )
        )
    return number
    ...
    for branch_id in parent_by_id:
        ...
        while current not in resolved:
            if current in path_indexes:
                raise ValueError(
                    "The parent-child table contains a cycle involving branch_id {}.".format(
                        branch_id
                    )
                )
            ...
            if parent_id is None:
                raise ValueError(
                    "Parent branch_id not found for branch_id {}.".format(branch_id)
                )
            if parent_id == -1:
                raise ValueError(
                    "branch_id {} is disconnected from the root.".format(branch_id)
                )
            if parent_id not in parent_by_id:
                raise ValueError(
                    "Parent branch_id {} was not found in the table.".format(parent_id)
                )
            current = parent_id
...
def _resolve_output_format(args, child_ids, records_by_id):
    internal_ids = {
        branch_id for branch_id, children in child_ids.items() if len(children) > 0
    }
    has_internal_names = any(
        records_by_id[branch_id]["name"] != "" for branch_id in internal_ids
    )
    has_internal_support = any(
        records_by_id[branch_id]["support"] is not None for branch_id in internal_ids
    )
...
def table2nwk_main(args):
    ...
    if missing_columns:
        raise ValueError(
            "Missing required column(s): {}".format(", ".join(missing_columns))
        )
    records = list()
    seen_branch_ids = set()
    root_ids = list()
    for row_idx, (_, row) in enumerate(table.iterrows()):
        ...
        record = {
            ...
            "dist": _parse_optional_float(row["dist"], "dist")
            if "dist" in row
            else None,
            "support": _parse_optional_float(row["support"], "support")
            if "support" in row
            else None,
            "row_order": row_idx,
        }
    ...
    if len(root_ids) != 1:
        raise ValueError(
            "The table must contain exactly one root row with parent = -1."
        )
    root_id = root_ids[0]
    ...
    for _branch_id, children in child_ids.items():
    ...
    for record in sorted(records, key=lambda rec: rec["row_order"]):
        ...
        if record["support"] is None:
            node.support = (
                None if record["branch_id"] == root_id else MISSING_SUPPORT_VALUE
            )
        else:

...
p_download.add_argument('--download_dir', metavar='PATH', default='auto', type=str, required=False, action='store',
...
p_table_input.add_argument('-o', '--outfile', metavar='PATH', default='-', type=str, required=False, action='store',
                           help='default=%(default)s: Output newick file. Use "-" for STDOUT.')
p_table_input.add_argument('-of', '--outformat', metavar='INT', default='auto', type=str, required=False, action='store',
...
def command_asr(args):
    ...
    asr_main(args)
pasr = subparsers.add_parser('asr', help='Infer categorical ancestral states and impute missing tip states under an Mk model', parents=[p_tree_input, p_table_output])
...
pasr.add_argument('--missing-values', '--missing_values', dest='missing_values', metavar='STR', default=None, type=str, required=False, action='store',
                  help='default=,NA,NaN,nan,?,missing,unknown: Comma-separated trait values treated as missing and imputed.')
...
pasr.add_argument('--target', metavar='all|intnode,missing_tip|tip', default='all', type=str, required=False, action='store',
                  help='default=%(default)s: Comma-separated node classes to report. '
                       'Use "missing_tip" to report only imputed tips. "leaf" is accepted as an alias for "tip".')
...
pasr.add_argument('--tree-outformat', '--tree_outformat', dest='tree_outformat', metavar='INT|auto', default='auto', type=str, required=False, action='store',
                  help='default=%(default)s: ETE tree format for --tree_out.')
pasr.add_argument('--tree-annotation', '--tree_annotation', dest='tree_annotation', metavar='state|probability|map|all', default='map', type=str, required=False, action='store',
                  ...
                  help='default=%(default)s: NHX properties written to --tree_out. '
...
pasr.add_argument('--n-sim', '--n_sim', dest='n_sim', metavar='INT', default=100, type=int, required=False, action='store',
                  help='default=%(default)s: Number of stochastic maps sampled when --stochastic_map_out is specified.')
...
pconstrain.add_argument('--species_list', metavar='PATH', default=None, type=str, required=False, action='store',
...
pconstrain.add_argument('--taxid_tsv', metavar='PATH', default=None, type=str, required=False, action='store',
                        help='default=%(default)s: TSV file containing species names in the "leaf_name" column and their NCBI Taxonomy IDs in the "taxid" column. '
                             ...
                             'Either --species_list or --taxid_tsv must be specified, but not both. '
...
pcollapse.add_argument('--min_support', metavar='FLOAT', default=None, type=float, required=False, action='store',
                       help='default=%(default)s: Collapse internal branches whose support is smaller than this threshold.')
pcollapse.add_argument('--max_dist', metavar='FLOAT', default=None, type=float, required=False, action='store',
                       help='default=%(default)s: Collapse internal branches whose branch length is at most this threshold.')
pcollapse.add_argument('--preserve_branch_length', metavar='yes|no', default='yes', type=strtobool, required=False, action='store',
...
pconsensus.add_argument('--method', metavar='greedy|majority|strict', default='greedy', type=str, required=False, action='store',
                        ...
                        help='default=%(default)s: Consensus-clade selection rule. '
                             'greedy uses --min_freq, majority keeps clades with frequency > 0.5, and strict keeps only clades present in all trees.')
...
def command_draw(args):
    ...
    draw_main(args)
pdraw = subparsers.add_parser('draw', help='Draw a phylogenetic tree with optional speciation/duplication node markers', parents=[p_tree_input, p_species])
...
pdraw.add_argument('--image_format', metavar='auto|pdf|png|svg', default='auto', type=str, required=False, action='store',
...
def command_info(args):
    ...
    info_main(args)
pinfo = subparsers.add_parser('info', help='Printing tree information', parents=[p_parent, p_species])
...
pimage.add_argument('--out_dir', metavar='PATH', default=None, type=str, required=True, action='store',
...
pimage.add_argument('--license_max', metavar='public-domain|cc-by|cc-by-sa|cc-by-nc|cc-by-nc-sa|any', default='cc-by-nc-sa', type=str, required=False, action='store',
...
pimage.add_argument('--allow_nd', metavar='yes|no', default='no', type=strtobool, required=False, action='store',
                    help='default=%(default)s: Whether ND licenses are acceptable.')
pimage.add_argument('--fallback_rank', metavar='none|genus|family', default='none', type=str, required=False, action='store',
                    choices=['none', 'genus', 'family'],
                    help='default=%(default)s: Allow fallback matching above the species rank.')
pimage.add_argument('--max_per_species', metavar='INT', default=1, type=int, required=False, action='store',
                    help='default=%(default)s: Number of images to keep per species.')
pimage.add_argument('--name_tsv', metavar='PATH', default=None, type=str, required=False, action='store',
                    help='default=%(default)s: Optional TSV with "leaf_name" and "species_name" columns.')
pimage.add_argument('--manifest', metavar='PATH', default=None, type=str, required=False, action='store',
                    help='default=%(default)s: Optional custom path for manifest.tsv.')
pimage.add_argument('--attribution', metavar='PATH', default=None, type=str, required=False, action='store',
                    help='default=%(default)s: Optional custom path for ATTRIBUTION.md.')
pimage.add_argument('--fail_on_missing', metavar='yes|no', default='no', type=strtobool, required=False, action='store',
                    help='default=%(default)s: Whether unresolved species should trigger a non-zero exit code.')
pimage.add_argument('--output_format', metavar='original|png|jpg', default='original', type=str, required=False, action='store',
...
pimage.add_argument('--max_edge', metavar='INT', default=None, type=int, required=False, action='store',
...
pimage.add_argument('--trim_shape', metavar='bbox|square', default='bbox', type=str, required=False, action='store',
                    choices=['bbox', 'square'],
...
prename.add_argument('--require-all-old-names', '--require_all_old_names', dest='require_all_old_names', metavar='yes|no', default='yes', type=strtobool, required=False, action='store',
                     help='default=%(default)s: Whether every old_name in --name_tsv must match a target node.')
...
def command_mcmctree(args):
    ...
    mcmctree_main(args)
pmcmctree = subparsers.add_parser('mcmctree', help=' Introduce divergence time constraints for PAML\'s mcmctree', parents=[p_parent, p_download, p_species])
...
def command_monophyly(args):
    ...
    monophyly_main(args)
pmonophyly = subparsers.add_parser('monophyly', help='Assess whether species or trait-defined groups are monophyletic', parents=[p_tree_input, p_table_output, p_species])
...
proot.add_argument('--taxonomy_source', metavar='ncbi[,opentree,timetree,...]', default='ncbi,opentree,timetree', type=str, required=False, action='store',
...
proot.add_argument('--taxid_tsv', metavar='PATH', default=None, type=str, required=False, action='store',
...
psanitize.add_argument('--resolve_polytomy', metavar='yes|no', default='no', type=strtobool, required=False, action='store',
...
def command_skim(args):
    ...
    skim_main(args)
pskim = subparsers.add_parser('skim', help='Sample leaves from clades with shared traits', parents=[p_parent])
...
pskim.add_argument('--filter-mode', '--filter_mode', dest='filter_mode', metavar='ascending|descending', default='ascending', type=str, required=False, action='store',
                       help='default=%(default)s: Sorting order for --filter_by values. '
...
def command_sample(args):
    ...
    sample_main(args)
psample = subparsers.add_parser('sample', help='Select a representative leaf subset by filters, ranks, and sampling method', parents=[p_parent])
...
psample.add_argument('--output-table', '--output_table', dest='output_table', metavar='PATH', default=None, type=str, required=False, action='store',

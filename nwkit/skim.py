...
def mark_traits_to_nodes(tree, trait_df, args):
    if (args.group_by is not None) and (args.group_by not in trait_df.columns):
        raise ValueError(
            "Column '{}' specified by '--group-by' was not found in '--trait'.".format(
                args.group_by
            )
        )
    if args.group_by is None:
        ...
        leafname2trait = {
            leaf_name: trait
            for leaf_name, trait in zip(
                trait_df["leaf_name"], trait_df[args.group_by], strict=True
            )
        }
...
def sample_from_groups(trait_df, args):
    if "group" not in trait_df.columns:
        raise ValueError(
            "Column 'group' not found. Run group assignment before sampling."
        )
    if (args.filter_by is not None) and (args.filter_by not in trait_df.columns):
        raise ValueError(
            "Column '{}' specified by '--filter-by' was not found in '--trait'.".format(
                args.filter_by
            )
        )
    ...
    if args.prioritize_non_missing and args.group_by is not None:
        if args.filter_by is None:
            sorted_df = shuffled_df.sort_values(
                by=[args.group_by], ascending=True, kind="mergesort"
            )
        else:
            if args.filter_mode == "ascending":
                ...
                raise ValueError(
                    f"Invalid value for '--filter-mode': {args.filter_mode}"
                )
    else:
        if args.filter_by is None:
    ...
    sampled_df = sorted_df.groupby("group", group_keys=False).head(
        args.retain_per_clade
    )
...
def add_contrastive_clade_ids(trait_df, marked_tree):
    ...
    for node in iter_contrastive_nodes(root=marked_tree.root):
        ...
        contrastive_clade_id += 1
    trait_df["contrastive_clade"] = (
        trait_df["leaf_name"].map(leafname2contrastive_clade).astype("Int64")
    )
    return trait_df
...
def skim_main(args):
    ...
    if getattr(args, "output_groupfile", False) and group_table_prefix in ["", None]:
        if args.outfile == "-":
            raise ValueError(
                "Legacy '--output-groupfile yes' requires '--outfile' to be a file path, not '-'."
            )
    ...
    validate_distinct_output_paths(
        [
            ...
            (
                "--group-table-prefix .all.tsv",
                None
                if group_table_prefix in ("", None)
                else "{}.all.tsv".format(group_table_prefix),
            ),
            (
                "--group-table-prefix .sampled.tsv",
                None
                if group_table_prefix in ("", None)
                else "{}.sampled.tsv".format(group_table_prefix),
            ),
        ]
    )
    tree = read_tree(
    ...
    if args.only_contrastive_clades:
        ...
        sampled_trait_df = sampled_trait_df[
            ~sampled_trait_df["contrastive_clade"].isna()
        ]
    else:
        sampled_trait_df = sample_from_groups(trait_df, args)

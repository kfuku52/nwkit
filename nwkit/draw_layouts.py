...
class TreeDrawingLayout:
    ...
    def bounds(self):
        points = [
            (float(x), float(y)) for path in self.edge_paths.values() for x, y in path
        ]
        points.extend((float(x), float(y)) for x, y in self.root_path)
        points.extend(
            (float(self.xcoord[node]), float(self.ycoord[node])) for node in self.xcoord
        )
        if not points:
...
def _normalized_leaf_weights(tree, leaf_weight_by_leaf=None):
    ...
    raw = {
        leaf: max(float(leaf_weight_by_leaf.get(leaf, 1.0)), 1e-9) for leaf in leaves
    }
    mean = sum(raw.values()) / max(len(raw), 1)
    return leaves, {leaf: value / max(mean, 1e-12) for leaf, value in raw.items()}
...
def _weighted_leaf_positions(leaves, normalized_weight):
    ...
    for previous, leaf in zip(leaves, leaves[1:], strict=False):
...
def get_rectangular_coordinates(
    ...
    for node in tree.traverse(strategy="postorder"):
        ...
        ycoord[node] = (
            float(sum(child_ys) / len(child_ys)) if child_ys else float(len(ycoord))
        )
...
def cladogram_layout(
    ...
    for node in tree.traverse(strategy="postorder"):
        ...
        height[node] = (
            0.0 if not children else 1.0 + max(height[child] for child in children)
        )
    total_height = height.get(tree, 0.0)
    xcoord = {node: total_height - height[node] for node in tree.traverse()}
    edge_paths, support_anchors, support_angles, root_path = _straight_paths(
        tree,
        xcoord,
        ycoord,
...
def _polar_coordinates(
    ...
    if count <= 1:
        ...
        seam_gap = (
            normalized_weight[first_at_seam] + normalized_weight[last_at_seam]
        ) / 2.0
        denominator = max(linear_span + seam_gap, 1e-12)
    angle = {
        node: start + (available * (linear_y[node] - linear_origin) / denominator)
        for node in tree.traverse()
    }
    xcoord = {node: radius[node] * math.cos(angle[node]) for node in tree.traverse()}
    ycoord = {node: radius[node] * math.sin(angle[node]) for node in tree.traverse()}
...
def _unrooted_center(adjacency):
    ...
    center = min(
        ...
        key=lambda node: (
            max(
                (component_weight(node, neighbor) for neighbor, _ in adjacency[node]),
                default=0,
            ),
            order_index[node],
        ),
...
def _covering_arc(origin, nodes, xcoord, ycoord):
    angles = sorted(
        math.atan2(ycoord[node] - origin[1], xcoord[node] - origin[0]) % (2.0 * math.pi)
        for node in nodes
        if math.hypot(
...
def _proper_segment_intersection(first, second, third, fourth, tolerance=1e-12):
    ...
    if first_side * second_side < -(tolerance**2) and third_side * fourth_side < -(
        tolerance**2
    ):
        return True
...
def _equal_daylight_adjust(
    ...
    def perform_pass(modifier):
        ...
        for pivot in internal_nodes:
            ...
            for index in range(1, len(arcs)):
                ...
                desired_midpoint += (previous_width / 2.0) + daylight + (width / 2.0)
                while actual_midpoint > desired_midpoint + math.pi:
                    actual_midpoint -= 2.0 * math.pi
    ...
    for iteration in range(1, max(int(max_iterations), 1) + 1):
        original = {node: (xcoord[node], ycoord[node]) for node in adjacency}
        total_change = 0.0
        adjustments = 0
        accepted = False
...
def unrooted_layout(
    ...
    while stack:
        ...
        total = float(
            sum(component_weight(node, neighbor) for neighbor, _ in neighbors)
        )
        if total <= 0.0:
            continue
        cursor = sector_start
    ...
    if suppressed_root is not None:
        ...
        xcoord[suppressed_root] = xcoord[first] + (
            (xcoord[second] - xcoord[first]) * fraction
        )
        ycoord[suppressed_root] = ycoord[first] + (
            (ycoord[second] - ycoord[first]) * fraction
        )
...
def _tidy_separate(box, index, index_y_list):
    ...
    while right is not None and left is not None:
        ...
        if index_y_list is None:
            break
        distance = (right_modifiers + right.prelim + right.w) - (
...
def _tidy_position_root(box):
    ...
    last = box.children[-1]
    box.prelim = (first.prelim + first.mod + last.mod + last.prelim + last.w) / 2.0 - (
        box.w / 2.0
    )
...
def _path_midpoint(path):
    ...
    for (start, end), length in zip(
        zip(path, path[1:], strict=False), lengths, strict=True
    ):
        if length <= 0.0:
            continue
...
def spiral_layout(
    ...
    global_fractions = sorted(
        {
            ...
            *((float(base.ycoord[node]) - y_min) / y_span for node in tree.traverse()),
        }
    )
    ...
    def vertical_sampler(start, end):
        ...
        return [
            (float(start[0]), y_min + (fraction * y_span)) for fraction in fractions
        ]
    ...
    support_anchors = {}
    support_angles = {}
    for node, _path in edge_paths.items():
        ...
        base_parent = node.up
        base_mid = (
            base.xcoord[base_parent]
            + ((base.xcoord[node] - base.xcoord[base_parent]) * 0.5),
            base.ycoord[node],
        )
...
def fractal_layout(
    ...
    xcoord = {node: (raw_x[node] - raw_center_x) * fit_x for node in raw_x}
    ycoord = {node: (raw_y[node] - raw_center_y) * fit_y for node in raw_y}
    edge_paths = {}
    support_anchors = {}
    support_angles = {}
    label_angles = {}
        ...
        if parent.is_root:
            ...
            polar_path = [
                (
                    parent_radius,
                    parent_angle
                    + ((child_angle - parent_angle) * index / (arc_samples - 1)),
                )
    ...
    if root_children:
        child_angles = sorted(
            incoming_angle.get(child, 0.0) % (2.0 * math.pi) for child in root_children
        )
        cyclic = child_angles + [child_angles[0] + (2.0 * math.pi)]
        gap_start, gap_end = max(
            zip(cyclic, cyclic[1:], strict=False),

...
def _draw_probability_pie(ax, x, y, probabilities, colors, marker_size_pt):
    ...
    for probability, color in zip(normalized, colors, strict=True):
        ...
        codes = (
            [MplPath.MOVETO] + ([MplPath.LINETO] * len(angles)) + [MplPath.CLOSEPOLY]
        )
        ax.scatter(
            [x],
            [y],
...
def _prepare_tree_figure(
    ...
    if use_topology_depth:
        sys.stderr.write(
            "Tree has no positive branch lengths; drawing x positions by topology depth.\n"
        )
    xcoord, ycoord, leaf_order = _get_tree_plot_coordinates(
        tree=tree, use_topology_depth=use_topology_depth
    )
    ...
    image_panel_width_in = (
        (tip_image_size_pt + 2.0 * tip_image_gap_pt) / 72.0
    ...
    if label_panel_width is None:
        label_panel_width_in = min(
            max(0.8, max_tip_label_chars * 0.07), main_panel_width_in * 0.58
        )
    else:
        label_panel_width_in = float(label_panel_width)
        if not 0.0 < label_panel_width_in < main_panel_width_in:
            raise ValueError(
    ...
    if tip_image_by_leaf:
        row_pitch_pt = max(row_pitch_pt, tip_image_size_pt + TIP_LABEL_GAP_PT)
    tree_panel_height_in = max(len(leaf_order), 1) * row_pitch_pt / 72.0
    legend_needed = bool(legend) and bool(
        node_type_by_node or group_color_by_name or property_legend_needed
    )
...
def _draw_tree_branches(
    tree, ax, xcoord, ycoord, x_span, branch_color, terminal_branch_color
):
    for node in tree.traverse():
        if node.is_leaf:
            continue
    ...
    for node in tree.traverse():
        ...
        edge_color = (
            ...
            if node.is_leaf and terminal_branch_color
            else branch_color
        )
        ax.plot(
...
def _draw_support_labels(
    tree, ax, xcoord, ycoord, support_labels, support_min, font_size, font_family
):
    ...
    for node in tree.traverse():
        ...
        label_x = parent_x + (xcoord[node] - parent_x) * 0.5
        ax.annotate(
...
def _tip_label_offset(
    ...
    return max(x_span * 0.02, 0.06, tip_pie_clearance), data_per_inch
...
def _draw_tip_labels(
    ...
    if tip_label_position not in ("branch-end", "aligned"):
        raise ValueError(
            "Unsupported '--tip-label-position': {}".format(tip_label_position)
        )
    for leaf in leaf_order:
        label_x = (
            xcoord[leaf] + label_offset
            if tip_label_position == "branch-end"
            else x_max + label_offset
        )
        ax.text(
            label_x,
            ycoord[leaf],
        ...
        label_width = max(len(leaf.name or ""), 1) * font_size * 0.58 / 72.0
        badge_width = max(len(value), 1) * font_size * 0.58 / 72.0
        badge_offset = (
            label_width + 4.0 / 72.0 + badge_width / 2.0 + font_size * 0.16 / 72.0
        ) * data_per_inch
        ax.text(
...
def _draw_root_marker(
    ...
    marker_size_pt = max(float(font_size), 5.0)
    if root_marker_size is not None:
        marker_size_pt = float(root_marker_size)
        if marker_size_pt <= 0.0:
            raise ValueError("--root-marker-size must be greater than zero.")
    if root_marker == "none":
        return marker_size_pt
...
def _configure_tree_axes(
    ...
    ax = layout["ax"]
    data_per_inch = layout["x_span"] / layout["tree_panel_width_in"]
    root_has_pie = (
    ...
    root_symbol_diameter_pt = root_marker_size_pt if root_marker != "none" else 0.0
    if root_has_pie:
        root_symbol_diameter_pt = max(root_symbol_diameter_pt, max(font_size, 4.5))
    root_symbol_margin = (
        (root_symbol_diameter_pt / 2.0 + 1.5) / 72.0 * data_per_inch
        if root_marker != "none" or root_has_pie
        else 0.0
    )
    label_panel_span = (
        layout["x_span"]
        * layout["label_panel_width_in"]
        / layout["tree_panel_width_in"]
    )
    ax.set_xlim(
        layout["x_min"] - max(root_stub * 1.2, root_symbol_margin),
        layout["x_max"] + label_offset + label_panel_span,
    )
    leaf_order = layout["leaf_order"]
    ax.set_ylim(0.5, -0.5) if not leaf_order else ax.set_ylim(
        float(len(leaf_order)) - 0.5, -0.5
    )
    ax.axis("off")
    axes_left = layout["left_margin_in"] / layout["fig_width"]
    axes_right = 1.0 - (
        (layout["right_margin_in"] + layout["image_panel_width_in"])
        / layout["fig_width"]
    )
    axes_top = 1.0 - layout["top_margin_in"] / layout["fig_height"]
    axes_bottom = layout["bottom_margin_in"] / layout["fig_height"]
    layout["fig"].subplots_adjust(
        left=axes_left, right=axes_right, top=axes_top, bottom=axes_bottom
    )
    if tip_image_by_leaf:
        image_ax = layout["fig"].add_axes(
            [
                axes_right,
                axes_bottom,
                layout["image_panel_width_in"] / layout["fig_width"],
        ...
        _draw_tip_images(
            ...
            image_size_pt=layout["tip_image_size_pt"],
        )


def _save_tree_figure(fig, outfile, image_format, transparent):
...
def _draw_tree(
    ...
    property_legend_needed = bool(tip_badge_property or node_pie_properties) and bool(
        property_colors
    )
    layout = _prepare_tree_figure(
        tree,
        figure_width,
        figure_height,
        label_panel_width,
    ...
    label_offset, data_per_inch = _tip_label_offset(
        layout["leaf_order"],
        layout["x_span"],
        layout["tree_panel_width_in"],
        font_size,
    ...
    _save_tree_figure(layout["fig"], outfile, image_format, transparent)

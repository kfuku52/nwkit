...
def _validated_calibration(record):
    ...
    if (
        ...
        raise ValueError('MCMCtree calibration lower age must not exceed its upper age.')
    for key in ('lower_tail', 'upper_tail'):
        if key in record and not 0.0 < float(record[key]) < 1.0:
            raise ValueError('MCMCtree calibration tail probabilities must be between 0 and 1.')
...
def parse_mcmctree_calibration(value):
    ...
    if match:
        ...
        return _validated_calibration({
            'raw': text, 'type': 'point', 'lower': age, 'upper': age,
        })
    ...
    if match:
        return _validated_calibration({
            'raw': text, 'type': 'lower', 'lower': float(match.group(1)),
        })
    match = _LEGACY_UPPER_PATTERN.fullmatch(text)
    if match:
        return _validated_calibration({
            'raw': text, 'type': 'upper', 'upper': float(match.group(1)),
        })
    return None
...
def read_mcmctree_posterior(source, tree, burnin=0, thin=1):
    ...
    if missing:
        raise ValueError(
            'MCMCtree posterior is missing node-age column(s): {}'.format(', '.join(missing))
        )
    retained = dataframe.iloc[burnin::thin]
    if retained.empty:
        raise ValueError('No MCMCtree posterior samples remain after burn-in and thinning.')
    try:
        values = retained.loc[:, expected_columns].to_numpy(dtype=float)
    except (TypeError, ValueError) as exc:
        raise ValueError('MCMCtree node-age columns must contain numeric values.') from exc
...
def summarize_mcmctree_posterior(tree, posterior, point='mean', ci='hpd', level=0.95):
    ...
    for node in tree.traverse():
        ...
        if distance < -1e-10:
            raise ValueError('Posterior point estimates produce a negative dated branch length.')
...
def infer_node_ages_from_branch_lengths(tree, tolerance=1e-5):
    ...
    for node in tree.traverse(strategy='postorder'):
        ...
        if max(child_ages) - min(child_ages) > max(tolerance, tolerance * max(child_ages)):
            raise ValueError(

...
from nwkit.image_metadata import (
    ...
    bioicons_display_author,
    bioicons_index_keys_for_name,
    bioicons_match_quality,
    canonical_license_url,
    classify_wikimedia_asset,
    infer_extension,
    infer_extension_from_bytes_prefix,
    license_allowed,
    ...
    search_text_mentions_query,
    strip_html_markup,
    wikimedia_page_mentions_query,
)
from nwkit.image_metadata import (
    SAFE_IMAGE_EXTENSIONS as SAFE_IMAGE_EXTENSIONS,
)
from nwkit.image_metadata import (
    canonicalize_search_terms as canonicalize_search_terms,
)
from nwkit.image_metadata import (
    infer_extension_from_content_type as infer_extension_from_content_type,
)
from nwkit.image_metadata import (
    infer_extension_from_response as infer_extension_from_response,
)
from nwkit.image_metadata import (
...
BIOICONS_GITHUB_API_ROOT = (
    "https://api.github.com/repos/duerrsimon/bioicons/git/trees/main"
)
...
def _hostname_matches_allowlist(hostname, allowed_hosts):
    ...
    return any(
        ...
        for allowed_host in (
            str(item).lower().rstrip(".") for item in allowed_hosts or ()
        )
    )
...
def _validate_public_ip_address(value, url):
    try:
        ...
        raise MediaDownloadError(
            "URL host has an invalid IP address: {!r}.".format(url)
        ) from exc
    if not address.is_global:
        raise MediaDownloadError(
            "URL resolves to a non-public address ({}) and was refused: {!r}.".format(
                address, url
            )
        )
...
def validate_external_url(url, allowed_hosts=None, resolve_dns=False):
    ...
    if parsed.scheme.lower() != "https":
        raise MediaDownloadError(
            "External image URLs must use HTTPS: {!r}.".format(url)
        )
    if parsed.username is not None or parsed.password is not None:
        raise MediaDownloadError(
            "External image URLs must not contain credentials: {!r}.".format(url)
        )
    hostname = parsed.hostname
    if hostname in (None, ""):
        raise MediaDownloadError(
            "External image URL has no hostname: {!r}.".format(url)
        )
    try:
        port = parsed.port
    except ValueError as exc:
        raise MediaDownloadError(
            "External image URL has an invalid port: {!r}.".format(url)
        ) from exc
    if port not in (None, 443):
        raise MediaDownloadError(
            "External image URL must use HTTPS port 443: {!r}.".format(url)
        )
...
def _request_pinned_address(
    session, method, url, address, suppress_sensitive_headers=False, **kwargs
):
    parsed = urlparse(url)
    ...
    pinned_session.hooks = {
        event: list(hooks) for event, hooks in session.hooks.items()
    }
    ...
    for redirect_count in range(MAX_SAFE_REDIRECTS + 1):
        ...
        if resolve_dns:
            ...
            if response is None:
                if last_error is None:
                    raise MediaDownloadError(
                        "No validated address was available for the external request."
                    )
                raise last_error
        else:
            try:
        ...
        if _redirect_changes_origin(current_url, next_url):
            ...
            if kwargs.get("headers") is not None:
                kwargs["headers"] = _safe_cross_origin_redirect_headers(
                    kwargs["headers"]
                )
...
def bounded_response_json(response, max_bytes=DEFAULT_MAX_PROVIDER_RESPONSE_BYTES):
    ...
    if content_length is not None and content_length > max_bytes:
        raise MediaDownloadError(
            "Provider response declares {} bytes, exceeding {}.".format(
    ...
    if hasattr(response, "iter_content"):
        ...
        for chunk in response.iter_content(chunk_size=64 * 1024):
            ...
            if total > max_bytes:
                raise MediaDownloadError(
                    "Provider response exceeded {} bytes.".format(max_bytes)
                )
            chunks.append(chunk)
        try:
            ...
            raise MediaDownloadError(
                "Provider returned invalid JSON: {}.".format(exc)
            ) from exc
...
def read_name_tsv(path):
    ...
    try:
        ...
        if missing:
            raise ValueError(
                '--species-name-tsv must contain "leaf_name" and "species_name" columns.'
            )
        mapping = dict()
        for row in reader:
            ...
            if leaf_name.strip() == "":
                raise ValueError(
                    '--species-name-tsv contains an empty "leaf_name" value.'
                )
            if leaf_name in mapping:
                raise ValueError(
                    'Duplicate values in the "leaf_name" column of --species-name-tsv are not supported.'
                )
            if species_name is None:
                raise ValueError(
                    '--species-name-tsv contains an empty "species_name" value.'
                )
...
def extract_species_mapping(
    tree, name_mapping=None, species_regex=DEFAULT_SPECIES_REGEX, args=None
):
...
def load_pillow_modules():
    try:
        ...
        raise RuntimeError(
            "Image validation and post-processing require Pillow. {}".format(
                PILLOW_INSTALL_HINT
            )
        ) from exc
...
def load_cairosvg_module():
    try:
        ...
        raise RuntimeError(
            ...
            "library. {}".format(CAIROSVG_INSTALL_HINT)
        ) from exc
    return cairosvg
...
def validate_image_dimensions(
    ...
    if width <= 0 or height <= 0:
        raise MediaDownloadError(
            "{} has non-positive dimensions {}x{}.".format(label, width, height)
        )
...
def inspect_svg_dimensions(source_path, enforce_limits=True):
    ...
    try:
        ...
        raise MediaDownloadError(
            "SVG image is not well-formed XML: {}.".format(exc)
        ) from exc
...
def validate_safe_svg(
    ...
    if (
        raw.startswith((b"\xff\xfe", b"\xfe\xff", b"\x00\x00\xfe\xff"))
        or b"\x00" in raw
    ):
        raise MediaDownloadError(
    ...
    def css_has_unsafe_reference(value):
        ...
        if "\\" in value:
            return True
        if re.search(
            r"(?:@import\b|expression\s*\(|(?:java|vb)script\s*:)",
            value,
            flags=re.IGNORECASE,
        ):
            return True
    ...
    for element_count, element in enumerate(root.iter(), start=1):
        ...
        if local_tag in forbidden_tags:
            raise MediaDownloadError(
                "SVG image contains forbidden <{}> content.".format(local_tag)
            )
        if local_tag == "style" and css_has_unsafe_reference(element.text):
            raise MediaDownloadError(
                "SVG image contains an external or executable CSS reference."
            )
        for raw_name, raw_value in element.attrib.items():
            ...
            if local_name.startswith("on"):
                raise MediaDownloadError(
                    "SVG image contains an event-handler attribute."
                )
            if local_name == "base":
            ...
            if (
                ...
                raise MediaDownloadError(
                    "SVG image contains an external resource reference."
                )
    ...
    if sanitized_raw != raw and rewrite_sanitized:
        ...
        try:
            with open(tmp_path, "wb") as write_handle:
                write_handle.write(sanitized_raw)
...
def estimate_background_color_from_border(rgb_image):
    ...
    if not samples:
        return rgb_image.getpixel((0, 0))
    buckets: defaultdict[tuple[int, int, int], list[int]] = defaultdict(
        lambda: [0, 0, 0, 0]
    )
...
def largest_component_bbox(mask, Image):
    ...
    if max(source_width, source_height) > SEMANTIC_MASK_MAX_DIM:
        if source_width >= source_height:
            ...
            working_height = max(
                1,
                int(round(source_height * SEMANTIC_MASK_MAX_DIM / float(source_width))),
            )
        else:
            working_height = SEMANTIC_MASK_MAX_DIM
            working_width = max(
                1,
                int(round(source_width * SEMANTIC_MASK_MAX_DIM / float(source_height))),
            )
        working_mask = cropped_mask.resize(
            (working_width, working_height), get_nearest_resampling_filter(Image)
        )
...
def semantic_foreground_bbox(image, Image, ImageChops):
    ...
    if alpha_bbox and alpha_max > 0 and alpha_min < 255:
        alpha_mask = alpha.point(
            lambda value: 255 if value > SEMANTIC_ALPHA_THRESHOLD else 0
        )
...
def trim_image(image, trim_mode, trim_shape, background_mode, Image, ImageChops):
    if trim_mode == "off":
        ...
        background = (
            rgb.getpixel((0, 0))
            if rgb.size[0] > 0 and rgb.size[1] > 0
            else (255, 255, 255)
        )
...
def square_canvas_image(image, background_mode, Image):
    if background_mode == "transparent":
        ...
        offset = (
            (canvas.size[0] - overlay.size[0]) // 2,
            (canvas.size[1] - overlay.size[1]) // 2,
        )
...
def save_processed_raster_image(image, destination_path):
    ...
    if image_format is None:
        raise RuntimeError(
            "Unsupported output extension for processed image: {}".format(
                dest_ext or "(none)"
            )
        )
    image_to_save = image
...
def rasterize_svg_to_image(source_path, max_edge=None):
    ...
    with Image.open(io.BytesIO(png_bytes)) as image:
        ...
        validate_image_dimensions(
            image_width, image_height, label="Rasterized SVG image"
        )
...
def process_raster_image(source_path, args):
    ...
    with Image.open(source_path) as source_image:
        ...
        validate_image_dimensions(source_width, source_height, label="Raster image")
        ...
        if can_downsample_before_decode:
            ...
            max_edge_value = int(max_edge)
            source_image.draft(source_image.mode, (max_edge_value, max_edge_value))
    ...
    destination_ext = output_extension_for_path(
        source_path, args=args, rasterized=False
    )
    if (
        destination_ext in (".jpg", ".jpeg")
        and getattr(args, "background", "white") == "transparent"
    ):
...
def validate_safe_raster(source_path):
    ...
    try:
        with Image.open(source_path) as image:
            image_width, image_height = image.size
            validate_image_dimensions(image_width, image_height, label="Raster image")
            image.verify()
        with Image.open(source_path) as image:
            image_width, image_height = image.size
            validate_image_dimensions(image_width, image_height, label="Raster image")
...
def process_svg_image(source_path, args):
    rasterized = rasterize_svg_to_image(
        source_path, max_edge=getattr(args, "max_edge", None)
    )
...
def resolve_image_query_cache_dir(args=None):
    ...
    if out_dir not in (None, ""):
        return os.path.join(
            os.path.realpath(out_dir), ".nwkit-cache", "image-query-cache"
        )
...
def cache_max_age_seconds_from_args(args):
    max_age_hours = getattr(
        args, "query_cache_max_age_hours", DEFAULT_QUERY_CACHE_MAX_AGE_HOURS
    )
...
def validate_candidate_media_url(candidate, resolve_dns=False):
    ...
    if provider not in SUPPORTED_SOURCES:
        raise MediaDownloadError(
            "Image provider candidate has an unknown provider {!r}.".format(provider)
        )
...
def normalize_provider_candidate(candidate, expected_provider=None):
    ...
    for field in (
        ...
        if not isinstance(value, (str, int, float)) or isinstance(value, bool):
            raise ValueError(
                "Image provider candidate has an invalid '{}' value.".format(field)
            )
...
def allowed_candidates_from_scored_candidates(candidates, args):
    return [
        candidate
        for candidate in dedupe_sorted_candidates(candidates)
        if license_allowed(
...
def should_stop_after_provider(candidates, args):
    ...
    allowed_candidates = allowed_candidates_from_scored_candidates(
    ...
    return all(
        candidate.get("matched_rank") == "species" for candidate in top_candidates
    )
...
def resolve_ncbi_taxonomy_image_cache_dir(args=None):
    ...
    if out_dir not in (None, ""):
        return os.path.join(
            os.path.realpath(out_dir), ".nwkit-cache", "ncbi-taxonomy-images"
        )
    ...
    try:
        ...
        for chunk in response.iter_content(chunk_size=4096):
            if chunk:
                ...
                if len(content) > 4096:
                    raise MediaDownloadError(
                        "NCBI checksum response is unexpectedly large."
                    )
        match = re.search(rb"\b([0-9a-fA-F]{32})\b", bytes(content))
        if match is None:
            raise MediaDownloadError(
                "NCBI checksum response did not contain an MD5 digest."
            )
...
def _download_to_path(
    ...
    try:
        ...
        if content_length is not None and content_length > int(max_bytes):
            raise MediaDownloadError(
                "NCBI archive declares {} bytes, exceeding {}.".format(
                    content_length, int(max_bytes)
                )
            )
        downloaded_bytes = 0
        ...
        if expected_md5 and digest.hexdigest().lower() != str(expected_md5).lower():
            raise MediaDownloadError(
                "Downloaded NCBI archive checksum did not match the published digest."
            )
        os.replace(tmp_path, destination_path)
    except Exception:
    ...
    try:
        with tarfile.open(tarball_path, "r:gz") as handle:
            ...
            if not member.isfile():
                raise MediaDownloadError(
                    "NCBI archive images.dmp entry is not a regular file."
                )
            if member.size <= 0 or member.size > NCBI_IMAGES_TABLE_MAX_BYTES:
                raise MediaDownloadError(
                    "NCBI images.dmp size {} is outside the supported range.".format(
                        member.size
                    )
                )
            ...
            if extracted is None:
                raise FileNotFoundError(
                    "images.dmp not found in {}".format(tarball_path)
                )
            try:
                with open(tmp_path, "wb") as out_handle:
                    ...
                    while True:
                        ...
                        if copied > NCBI_IMAGES_TABLE_MAX_BYTES:
                            raise MediaDownloadError(
                                "Extracted NCBI images.dmp exceeded its size limit."
                            )
...
def _ncbi_images_database_is_valid(path):
    try:
        if os.path.getsize(path) <= 0:
            return False
        uri = "file:{}?mode=ro".format(quote(os.path.abspath(path), safe="/"))
        ...
        try:
            ...
            if not {"taxid", "record_id", "image_url"}.issubset(columns):
                return False
            return (
                connection.execute("SELECT 1 FROM images LIMIT 1").fetchone()
                is not None
            )
        finally:
            connection.close()
...
def ensure_ncbi_images_table(args, session):
    ...
    with acquire_exclusive_lock(lock_path=lock_path, lock_label="NCBI taxonomy images"):
        ...
        try:
            ...
            expected_md5 = None
            _stderr(
                "Warning: could not verify the published NCBI archive checksum: {}".format(
                    exc
                )
            )
        ...
        if os.path.isfile(tarball_path):
            try:
                archive_is_valid = 0 < os.path.getsize(
                    tarball_path
                ) <= NCBI_NEWTAXDUMP_MAX_BYTES and (
                    expected_md5 is None or _file_md5(tarball_path) == expected_md5
                )
                if archive_is_valid:
        ...
        try:
            _extract_ncbi_images_table(
                tarball_path=tarball_path, destination_path=images_path
            )
            ...
            try:
                ...
                if not _ncbi_images_table_is_valid(images_path):
                    raise MediaDownloadError(
                        "Extracted NCBI images.dmp failed validation."
                    )
            except Exception:
                remove_temporary_path(tarball_path, "invalid NCBI archive")
                remove_temporary_path(
                    images_path, "invalid extracted NCBI images table"
                )
                raise
        try:
...
def ensure_ncbi_images_database(args, session):
    ...
    with acquire_exclusive_lock(
        lock_path=lock_path, lock_label="NCBI taxonomy images database"
    ):
        if _ncbi_images_database_is_valid(database_path):
            return database_path
...
class PhylopicProvider:
    ...
    def fetch_candidates(self, species_name, fallback_rank="none"):
        candidates = list()
        for matched_rank, query_name in get_taxonomic_queries(
            species_name, fallback_rank=fallback_rank, ncbi=self.ncbi
        ):
            ...
            if primary_uuid and not any(
                item.get("uuid") == primary_uuid for item in image_items
            ):
            ...
            for image_item in image_items:
                candidate = self._candidate_from_image(
                    ...
                    is_primary=bool(
                        primary_uuid and image_item.get("uuid") == primary_uuid
                    ),
                )
    ...
    def _candidate_from_image(
        self, image_item, matched_name, matched_rank, is_primary=False
    ):
        ...
        if selected_file is None and raster_files:
            selected_file = max(
                raster_files,
                key=lambda item: max(
                    parse_size(item.get("sizes"))[0] or 0,
                    parse_size(item.get("sizes"))[1] or 0,
                ),
            )
        ...
        license_code = normalize_license_code(
            raw_url=license_url, attribution=image_item.get("attribution")
        )
...
class BioiconsProvider:
    ...
    def _load_catalog(self):
        ...
        if catalog is None:
            ...
            with acquire_exclusive_lock(
                lock_path=lock_path, lock_label="Bioicons catalog"
            ):
                catalog = load_memory_catalog()
                if catalog is None:
                    catalog = load_cached_bioicons_catalog(
                    ...
                    if catalog is None:
                        ...
                        for item in payload.get("tree", []):
                            path = item.get("path", "")
                            if (not path.startswith("static/icons/")) or (
    ...
    def fetch_candidates(self, species_name, fallback_rank="none"):
        ...
        for matched_rank, query_name in get_taxonomic_queries(
            ...
            for alias in BIOICONS_SPECIES_ALIASES.get(
            ...
            for icon in candidate_icons:
                ...
                media_url = "{}/{}".format(
                    BIOICONS_MEDIA_ROOT.rstrip("/"), relative_path
                )
                candidates.append(
                    {
                        "provider": self.provider_name,
...
class INaturalistProvider:
    ...
    def __init__(self, session, ncbi=None, args=None):
        self.session = session
        self.ncbi = ncbi
        self.result_limit = resolve_provider_fetch_limit(
            args=args, minimum=10, maximum=30
        )
    ...
    def _find_taxon(self, query_name, expected_rank):
        ...
        matches = [
            ...
            and item.get("rank") == expected_rank
        ]
        if matches:
            return matches[0]
        return None
    ...
    def fetch_candidates(self, species_name, fallback_rank="none"):
        ...
        for matched_rank, query_name in get_taxonomic_queries(
            ...
            observations = self._fetch_observations(
                taxon_id=taxon["id"], per_page=per_page
            )
    ...
    def _candidate_from_photo(self, photo, observation, taxon, matched_rank):
        ...
        license_code = normalize_license_code(
            raw_code=photo.get("license_code"), attribution=photo.get("attribution")
        )
        quality_grade_bonus = {
            "research": 30,
            "needs_id": 10,
        ...
        popularity_bonus = min(
            int(
                observation.get("cached_votes_total")
                or observation.get("faves_count")
                or 0
            ),
            20,
        )
...
class EOLProvider:
    ...
    def __init__(self, session, ncbi=None, args=None):
        ...
        self.page_limit = resolve_provider_fetch_limit(
            args=args, minimum=3, maximum=5, extra_buffer=0
        )
    ...
    def _candidate_from_data_object(self, data_object, matched_name, matched_rank):
        ...
        creator_names = [
            ...
            if str(agent.get("role", "")).lower()
            in ("creator", "photographer", "illustrator")
        ]
        rights_holder = strip_html_markup(data_object.get("rightsHolder", ""))
        attribution = rights_holder or ", ".join(
            [name for name in creator_names if name != ""]
        )
        license_code = normalize_license_code(
        ...
        return {
            ...
            "provider_record_id": str(
                data_object.get("dataObjectVersionID")
                or data_object.get("identifier", "")
            ),
            "matched_name": matched_name,
            "matched_rank": matched_rank,
            "license_code": license_code,
            "license_url": license_value
            if "://" in str(license_value)
            else canonical_license_url(license_code),
            "attribution": attribution,
...
class IDigBioProvider:
    ...
    def _build_record_query(self, query_name, matched_rank):
        ...
        raise ValueError(
            "Unsupported matched rank for iDigBio: {}".format(matched_rank)
        )

    def _search_media(self, query_name, matched_rank, limit=None):
        body = {
            "rq": self._build_record_query(
                query_name=query_name, matched_rank=matched_rank
            ),
    ...
    def fetch_candidates(self, species_name, fallback_rank="none"):
        ...
        for matched_rank, query_name in get_taxonomic_queries(
            ...
            for item in self._search_media(
                query_name=query_name, matched_rank=matched_rank
            ):
    ...
    def _candidate_from_item(self, item, matched_name, matched_rank):
        ...
        media_url = (
            data.get("ac:accessURI")
            or data.get("dcterms:identifier")
            or index_terms.get("accessuri")
        )
        if not media_url:
            return None

        media_kind = str(
            index_terms.get("mediatype")
            or index_terms.get("type")
            or data.get("dc:type")
            or ""
        ).lower()
        ...
        scientific_name = normalize_species_name(
            data.get("dwc:scientificName") or matched_name
        )

        license_url = (
            data.get("xmpRights:UsageTerms") or data.get("xmpRights:WebStatement") or ""
        )
        license_code = normalize_license_code(
            raw_code=data.get("dcterms:rights") or data.get("dc:rights") or "",
            raw_url=license_url,
            attribution=data.get("xmpRights:Owner")
            or data.get("ac:providerLiteral")
            or "",
        )
        try:
            width = (
                int(data.get("exif:PixelXDimension") or index_terms.get("xpixels") or 0)
                or None
            )
        except (TypeError, ValueError):
            width = None
        try:
            height = (
                int(data.get("exif:PixelYDimension") or index_terms.get("ypixels") or 0)
                or None
            )
        except (TypeError, ValueError):
            height = None
        ...
        exact_name_bonus = (
            ...
            if scientific_name
            and (
                scientific_name.lower() == normalize_species_name(matched_name).lower()
            )
            else 0
        )
        return {
            ...
            "license_code": license_code,
            "license_url": license_url
            if "://" in str(license_url)
            else canonical_license_url(license_code),
            "attribution": data.get("xmpRights:Owner")
            or data.get("ac:providerLiteral")
            or "",
            "source_page_url": data.get("dcterms:identifier")
            or data.get("xmpRights:WebStatement")
            or media_url,
            "media_url": media_url,
            "width": width,
            "height": height,
...
class OpenverseProvider:
    ...
    def _candidate_from_result(self, result, matched_name, matched_rank):
        ...
        matched_fields = [
            str(field).lower() for field in result.get("fields_matched", []) or []
        ]
        provider_quality = 0
        ...
        return {
            ...
            or canonical_license_url(license_code),
            "attribution": result.get("creator", "") or "",
            "source_page_url": result.get("foreign_landing_url")
...
class WikimediaProvider:
    ...
    def __init__(self, session, ncbi=None, args=None):
        ...
        self.ncbi = ncbi
        self.result_limit = resolve_provider_fetch_limit(
            args=args, minimum=5, maximum=10
        )
        ...
        for matched_rank, query_name in get_taxonomic_queries(
            ...
            for page in self._search_pages(query_name=query_name):
                candidate = self._candidate_from_page(
                    page=page, matched_name=query_name, matched_rank=matched_rank
                )
                if candidate is not None:
                    candidates.append(candidate)
        return candidates
    ...
    def _candidate_from_page(self, page, matched_name, matched_rank):
        ...
        license_short_name = strip_html_markup(
...
class GBIFProvider:
    ...
    def _candidate_from_media(self, occurrence, media, matched_name, matched_rank):
        ...
        provider_record_id = (
            media.get("references") or occurrence.get("key") or media_url
        )
        provider_quality = 0
        ...
        return {
            ...
            "license_url": license_url,
            "attribution": attribution,
            "source_page_url": media.get("references")
...
class NCBIProvider:
    ...
    def _ensure_images_database(self):
        if self.images_database_path is not None:
            return
        self.images_database_path = ensure_ncbi_images_database(
            args=self.args, session=self.session
        )
    ...
    def _candidate_from_record(self, record, matched_name, matched_rank):
        ...
        attribution = ", ".join(
            [bit for bit in attribution_bits if bit not in ("", None)]
        )
        provider_quality = 0
...
def build_providers(args, sources, session=None):
    ...
    for source in sources:
        if source == "phylopic":
            if ncbi is None:
                raise ValueError(
                    "PhyloPic lookups require an initialized taxonomy database."
                )
            ...
            providers[source] = INaturalistProvider(
                session=session, ncbi=ncbi, args=args
            )
            ...
            if ncbi is None:
                raise ValueError(
                    "NCBI lookups require an initialized taxonomy database."
                )
...
def collect_candidates_for_species(species_name, args, sources, providers):
    ...
    for provider_index, source in enumerate(sources):
        ...
        fetch_limit = int(
            getattr(provider, "result_limit", resolve_provider_fetch_limit(args=args))
        )
        ...
        try:
            ...
            if provider_candidates is None:
                provider_candidates = provider.fetch_candidates(
                    species_name, fallback_rank=args.fallback_rank
                )
        ...
        for candidate_index, candidate in enumerate(provider_candidates, start=1):
            try:
                ...
                message = "{} candidate {} was skipped for {}: {}".format(
                    source,
                    candidate_index,
                    species_name,
                    exc,
...
def validate_media_file(path, max_download_bytes=DEFAULT_MAX_DOWNLOAD_BYTES):
    try:
        ...
        raise MediaDownloadError(
            "Downloaded media is unavailable: {}".format(exc)
        ) from exc
    if size <= 0:
    ...
    if extension == "":
        raise MediaDownloadError(
            "Downloaded response is not a recognized JPEG, PNG, GIF, TIFF, WebP, or SVG image."
        )
    return extension
...
def response_content_type_is_plausible_image(response):
    headers = getattr(response, "headers", {}) or {}
    content_type = (
        str(headers.get("Content-Type") or "").split(";", 1)[0].strip().lower()
    )
    return (
        content_type == ""
...
def download_media(
    ...
    if cache_path is not None:
        ...
        if os.path.exists(cache_path) and os.path.getsize(cache_path) > 0:
            try:
                ...
                resolved_destination_path = replace_extension(
                    destination_path, cache_ext
                )
    ...
    try:
        ...
        if not response_content_type_is_plausible_image(response):
            ...
            raise MediaDownloadError(
                "Media URL returned non-image Content-Type {!r}.".format(content_type)
            )
        content_length = response_content_length(response)
            ...
            for chunk in response.iter_content(chunk_size=1024 * 64):
                if chunk:
                    ...
                    if downloaded_bytes > max_download_bytes:
                        raise MediaDownloadError(
                            "Media response exceeded --max-download-bytes {}.".format(
                                max_download_bytes
                            )
                        )
                    handle.write(chunk)
        resolved_ext = validate_media_file(
            tmp_path, max_download_bytes=max_download_bytes
        )
...
def default_output_paths(args):
    ...
    manifest_path = (
        os.path.realpath(manifest_out)
        if manifest_out
        else os.path.join(out_dir, "manifest.tsv")
    )
    attribution_path = (
        os.path.realpath(attribution_out)
        if attribution_out
        else os.path.join(out_dir, "ATTRIBUTION.md")
    )
...
def write_attribution_markdown(path, selected_assets):
    ...
    for local_path in sorted(grouped.keys()):
        ...
        for index, record in enumerate(attribution_records, start=1):
            ...
            lines.append(
                "License: {}".format(_safe_markdown_text(record["license_code"]))
            )
            if record["license_url"]:
                lines.append(
                    "License URL: {}".format(_safe_markdown_text(record["license_url"]))
                )
            if record["source_page_url"]:
                lines.append(
                    "Source: {}".format(_safe_markdown_text(record["source_page_url"]))
                )
            lines.append("")
...
def validate_args(args):
    ...
    auxiliary_outputs = {
        "--manifest-out": getattr(args, "manifest_out", None)
        or getattr(args, "manifest", None),
        "--attribution-out": getattr(args, "attribution_out", None)
    ...
    if (getattr(args, "output_format", "original") == "jpg") and (
        getattr(args, "background", "white") == "transparent"
    ):
        raise ValueError(
            "--background transparent is incompatible with --output-format jpg."
        )
...
def resolve_lookup_worker_count(args, sources, species_count):
    ...
    uses_taxonomy_db = (
        ("phylopic" in sources)
        or ("ncbi" in sources)
        or (getattr(args, "fallback_rank", "none") == "family")
    )
...
def collect_candidates_for_species_map(species_names, args, sources):
    ...
    if len(species_names) == 0:
        return dict()
    max_workers = resolve_lookup_worker_count(
        args=args, sources=sources, species_count=len(species_names)
    )
    if max_workers <= 1:
    ...
    species_batches = [
        species_names[index::max_workers] for index in range(max_workers)
    ]
...
class SharedMediaMaterializer:
    ...
    def _download_candidate(self, media_url):
        ...
        download_result = download_media(
            ...
            max_download_bytes=getattr(
                self.args, "max_download_bytes", DEFAULT_MAX_DOWNLOAD_BYTES
            ),
            provider=candidate.get("provider"),
        ...
        if isinstance(download_result, dict):
            ...
            absolute_local_path = download_result.get(
                "destination_path", absolute_local_path
            )
        ...
        try:
            absolute_local_path = postprocess_media_file(
                absolute_local_path, args=self.args
            )
        except (MediaDownloadError, OSError, ValueError):
            remove_temporary_path(
                absolute_local_path, "invalid materialized media file"
            )
            if resolved_cache_path and os.path.realpath(
                resolved_cache_path
            ) != os.path.realpath(absolute_local_path):
    ...
    def materialize(self, species_name, candidate):
        ...
        plan_entry = self.media_plan.get(
            media_url, {"species_name": species_name, "candidate": candidate}
        )
        owner = False
        with self._lock:
        ...
        if error is not None:
            if not isinstance(error, BaseException):
                raise RuntimeError("Media materialization failed without an exception.")
            raise error
        task_result = task["result"]
        if task_result is None:
            raise RuntimeError("Media materialization completed without a result.")
...
def build_media_plan(species_names, lookup_results, args):
    ...
    for species_name in species_names:
        ...
        for candidate in allowed_candidates_from_scored_candidates(
            candidates, args=args
        ):
            media_url = candidate["media_url"]
    ...
    for candidate in allowed_candidates:
        ...
        try:
            materialized = materializer.materialize(
                species_name=species_name, candidate=candidate
            )
        except (
...
def image_main(args):
    ...
    out_dir, images_dir, manifest_path, attribution_path, unmatched_path = (
    ...
    species_name_tsv = getattr(args, "species_name_tsv", None) or getattr(
        args, "name_tsv", None
    )
    ...
    lookup_results = collect_candidates_for_species_map(
        species_names=species_names, args=args, sources=sources
    )
    materializer = None
    try:
        media_plan = build_media_plan(
            species_names=species_names, lookup_results=lookup_results, args=args
        )
        ...
        max_download_workers = resolve_download_worker_count(
        ...
        for species_name in species_names:
            species_manifest_rows, species_selected_assets, species_unmatched_rows = (
                download_results[species_name]
            )
    ...
    if unmatched_rows and args.fail_on_missing:
        raise ValueError(
            "{} tree tip(s) could not be resolved to an image.".format(
                len(unmatched_rows)
            )
        )

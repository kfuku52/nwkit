...
class CalibratedSearch:
    def __init__(
        ...
        self.models, self.counts = enumerate_candidates(tree, max_shifts, limit)
        if not convergence:
            self.models = [
                m
                for m in self.models
                if len(m["groups"]) == len(m["shift_branch_ids"]) + 1
            ]
        ...
        for ai, alpha in enumerate(self.grid):
            ...
            for variance in self.variance_grid:
                ...
                G = np.einsum("mdi,mdj->mij", W, W)
                inverse = np.zeros_like(G)
                one, two = self.dim == 1, self.dim == 2
                inverse[one, 0, 0] = 1 / G[one, 0, 0]
                inverse[two] = np.linalg.inv(G[two])
                self.cache.append(
                    (ai, variance, L, W, inverse, 2 * np.log(np.diag(L)).sum())
                )
    ...
    def _at(self, Z, item):
        _, _, L, W, inverse, logdet = item
        w = solve_triangular(L, Z, lower=True)
        dots = np.einsum("mdi,db->mib", W, w)
        beta = np.einsum("mij,mjb->mib", inverse, dots)
    ...
    def fit(self, values, seed=1, replicates=199, level=0.05):
        if replicates < 19 or not 0 < level < 1 or 1 / (replicates + 1) > level:
            raise ValueError(
                "Calibration needs >=19 replicates and Monte Carlo resolution no larger than the test level."
            )
        ...
        for stage, family in enumerate(self.families):
            ...
            sim = mean[:, None] + np.sqrt(scale) * L @ rng.normal(
                size=(self.d, replicates)
            )
            sb, _ = self.profile(sim)
            observed = float(2 * (best.max() - best[winner]))
            statistics = 2 * (sb.max(axis=0) - sb[family].max(axis=0))
            p = float(
                (1 + np.count_nonzero(statistics >= observed - 1e-10))
                / (replicates + 1)
            )
            tests.append(
                {
        ...
        for candidate_item in self.cache:
            score, _, _ = self._at(z[:, None], candidate_item)
            index = candidate_item[0]
            profile[index] = max(profile[index], score[winner, 0])

...
def main():
    ...
    for path, sha in design["source_sha256"].items():
        ...
        if (
            ...
            raise ValueError(f"Active fitting/generator source hash mismatch: {path}")
    for row in rows:
    ...
    audit_records(rows, design, summary, replay_bootstrap=args.replay_bootstrap)
    ...
    report = {
        ...
        "source_hashes": "snapshots_and_active_fitting_sources_match",
        "bootstrap_probabilities_replayed": len(rows) if args.replay_bootstrap else 0,

...
def audit_record(row, design, search):
    ...
    best, _ = search.profile(search.q @ (values - values.mean()))
    best = best[:, 0]
    tests = fit["tests"]
...
def audit_records(rows, design, summary):
    ...
    for row in rows:
        ...
        result = audit_record(row, design, engines[key])

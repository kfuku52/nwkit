...
from nwkit.image import extract_species_mapping
from nwkit.image import read_name_tsv as read_species_name_tsv
...
def test_help_hides_snake_case_compatibility_aliases(capsys):
    for command, command_parser in _command_parsers().items():
        assert (
            re.search(r"--[A-Za-z0-9-]*_[A-Za-z0-9_-]*", command_parser.format_help())
            is None
        ), command
...
def test_deprecated_option_aliases_warn_with_canonical_replacements(tmp_path, capsys):
    ...
    assert (
        _deprecated_option_aliases("skim")["--output-groupfile"]
        == "--group-table-prefix"
    )
    ...
    captured = capsys.readouterr()
    warning_lines = [
        line for line in captured.err.splitlines() if line.startswith("Warning:")
    ]
...
def test_specialized_tsv_inputs_support_standard_input(monkeypatch, tmp_path):
    ...
    assert set(read_tree(str(outfile), "1", True, quiet=True).leaf_names()) == {
        "A",
        "B",
    }
...
def test_constrain_validates_standard_input_only_once(monkeypatch):
    ...
    labels, taxid_df = check_input_file(
        Namespace(species_list="-", taxid_tsv=None, backbone="user")
    )
    ...
    monkeypatch.setattr(sys, "stdin", io.StringIO("leaf_name\ttaxid\nA_a\t1\n"))
    labels, taxid_df = check_input_file(
        Namespace(species_list=None, taxid_tsv="-", backbone="ncbi")
    )

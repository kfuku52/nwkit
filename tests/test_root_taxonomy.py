...
class TestTaxonomyRooting:
    ...
    def test_ncbi_source_passes_args_to_ncbi_helpers(self, monkeypatch, tmp_path):
        ...
        def fake_resolve_ncbi_lineages(
        ...
        monkeypatch.setattr(
            root_mod, "_resolve_ncbi_lineages", fake_resolve_ncbi_lineages
        )
    ...
    def test_infers_ncbi_lineages_from_leaf_labels(self, monkeypatch):
        ...
        rooted = taxonomy_rooting(
            tree, taxonomy_source="ncbi", taxid_tsv=None, rank="no"
        )
        child_leaf_sets = [set(child.leaf_names()) for child in rooted.get_children()]
    ...
    def test_ncbi_taxonomic_uses_taxonomy_query_fallbacks(self, monkeypatch):
        ...
        class FakeNCBI:
            ...
            def get_taxid_translator(self, taxids):
                ...
                return {
                    int(taxid): names[int(taxid)]
                    for taxid in taxids
                    if int(taxid) in names
                }
            ...
            def get_rank(self, taxids):
                ...
                return {
                    int(taxid): ranks.get(int(taxid), "no rank") for taxid in taxids
                }
        ...
        rooted = taxonomy_rooting(
            tree, taxonomy_source="ncbi", taxid_tsv=None, rank="no", args=args
        )
        child_leaf_sets = [set(child.leaf_names()) for child in rooted.get_children()]
        assert {
    ...
    def test_taxid_tsv_requires_exact_leaf_label_match(self, monkeypatch, tmp_path):
        ...
        with pytest.raises(ValueError, match="match the leaf labels"):
            taxonomy_rooting(
                tree, taxonomy_source="ncbi", taxid_tsv=str(tsv_path), rank="no"
            )

    def test_taxid_tsv_numeric_leaf_label_mismatch_raises_valueerror(
        ...
        tsv_path = tmp_path / "taxid.tsv"
        pd.DataFrame({"leaf_name": [1, 3], "taxid": [100, 200]}).to_csv(
    ...
    def test_taxid_tsv_numeric_leaf_labels_are_accepted(self, monkeypatch, tmp_path):
        ...
        tsv_path = tmp_path / "taxid.tsv"
        pd.DataFrame({"leaf_name": [1, 2, 3], "taxid": [100, 101, 200]}).to_csv(
            tsv_path, sep="\t", index=False
        )
    ...
    def test_taxid_tsv_leading_zero_leaf_labels_are_accepted(
        self, monkeypatch, tmp_path
    ):
        install_fake_ncbi(
            monkeypatch,
    ...
    def test_taxid_tsv_na_literal_leaf_labels_are_accepted(self, monkeypatch, tmp_path):
        ...
        tsv_path = tmp_path / "taxid.tsv"
        pd.DataFrame({"leaf_name": ["NA", "B", "C"], "taxid": [100, 101, 200]}).to_csv(
            tsv_path, sep="\t", index=False
        )
    ...
    def test_ambiguous_taxonomy_root_raises(self, monkeypatch, tmp_path):
        ...
        tsv_path = tmp_path / "taxid.tsv"
        pd.DataFrame({"leaf_name": ["A", "B", "C"], "taxid": [100, 200, 300]}).to_csv(
            tsv_path, sep="\t", index=False
        )
    ...
    def test_incompatible_taxonomy_bipartition_raises(self, monkeypatch):
        ...
        tree = Tree(
            "((Homo_sapiens:1,Arabidopsis_thaliana:1):1,(Pan_troglodytes:1,Oryza_sativa:1):1);",
            parser=1,
        )
    ...
    def test_timetree_allows_monophyletic_duplicate_species_labels(self, monkeypatch):
        ...
        assert {
            "Homo_sapiens_gene1",
            "Homo_sapiens_gene2",
            "Pan_troglodytes_gene1",
        } in child_leaf_sets
        assert {
            "Arabidopsis_thaliana_gene1",
            "Oryza_sativa_gene1",
            "Saccharomyces_cerevisiae_gene1",
        } in child_leaf_sets

    def test_timetree_unresolved_names_are_excluded_when_not_interfering(
        self, monkeypatch
    ):
        install_fake_timetree(
            monkeypatch,
    ...
    def test_source_chain_falls_back_to_timetree(self, monkeypatch):
        ...
        rooted = taxonomy_rooting(
            tree, taxonomy_source="ncbi,timetree", taxid_tsv=None, rank="no"
        )
        child_leaf_sets = [set(child.leaf_names()) for child in rooted.get_children()]
        assert {"Homo_sapiens", "Pan_troglodytes"} in child_leaf_sets
        assert {"Arabidopsis_thaliana", "Oryza_sativa"} in child_leaf_sets
    ...
    def test_opentree_unresolved_name_is_excluded_when_not_interfering(
        self, monkeypatch
    ):
        install_fake_opentree(
            monkeypatch,
    ...
    def test_source_chain_falls_back_to_opentree(self, monkeypatch):
        ...
        rooted = taxonomy_rooting(
            tree, taxonomy_source="ncbi,opentree", taxid_tsv=None, rank="no"
        )
        child_leaf_sets = [set(child.leaf_names()) for child in rooted.get_children()]
        assert {"Homo_sapiens", "Pan_troglodytes"} in child_leaf_sets
        assert {"Arabidopsis_thaliana", "Oryza_sativa"} in child_leaf_sets

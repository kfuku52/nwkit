...
class TestDrawMain:
    ...
    def test_species_overlap_node_types_use_descendant_species_once(self):
        tree = Tree(
            "(((Homo_sapiens_G1:1,Homo_sapiens_G2:1):1,Mus_musculus_G1:1):1,Danio_rerio_G1:1);",
            parser=1,
        )
        args = make_draw_args()
        ...
        assert (
            node_type_by_node[
                tree.common_ancestor(["Homo_sapiens_G1", "Homo_sapiens_G2"])
            ]
            == "duplication"
        )
        assert (
            node_type_by_node[
                tree.common_ancestor(["Homo_sapiens_G1", "Mus_musculus_G1"])
            ]
            == "speciation"
        )
    ...
    def test_draw_skips_species_overlap_legend_when_labels_do_not_match_regex(
    ...
    def test_draw_skips_species_overlap_legend_for_unrooted_tree_in_auto_mode(
        self, tmp_nwk, tmp_path, capsys
    ):
        infile = tmp_nwk("(Homo_sapiens_G1:1,Mus_musculus_G1:1,Danio_rerio_G1:1);")
        outfile = tmp_path / "unrooted_auto.svg"
        args = make_draw_args(
            infile=infile, outfile=str(outfile), species_overlap_node_plot="auto"
        )
    ...
    def test_draw_raises_for_unrooted_tree_in_yes_mode(self, tmp_nwk, tmp_path):
        ...
        args = make_draw_args(
            infile=infile, outfile=str(outfile), species_overlap_node_plot="yes"
        )
    ...
    def test_draw_uses_tight_tip_label_spacing(self, tmp_nwk, tmp_path):
        ...
        args = make_draw_args(
            infile=infile, outfile=str(outfile), species_overlap_node_plot="no"
        )
    ...
    def test_draw_colors_tip_labels_from_trait_table(self, tmp_nwk, tmp_path):
        ...
        pd.DataFrame({"leaf_name": ["A", "B", "C"], "group": ["x", "x", "y"]}).to_csv(
            trait_path, sep="\t", index=False
        )
    ...
    def test_draw_rejects_unknown_trait_leaf_names(self, tmp_nwk, tmp_path):
        ...
        trait_path = tmp_path / "traits.tsv"
        pd.DataFrame({"leaf_name": ["A", "Z"], "group": ["x", "y"]}).to_csv(
            trait_path, sep="\t", index=False
        )
        outfile = tmp_path / "trait.svg"
        args = make_draw_args(
    ...
    def test_draw_accepts_numeric_leaf_names_in_trait_table(self, tmp_nwk, tmp_path):
        ...
        trait_path = tmp_path / "traits.tsv"
        pd.DataFrame({"leaf_name": [1, 2], "group": ["x", "y"]}).to_csv(
            trait_path, sep="\t", index=False
        )
    ...
    def test_draw_accepts_leading_zero_leaf_names_in_trait_table(
        self, tmp_nwk, tmp_path
    ):
        infile = tmp_nwk("((001:1,002:1):1,003:1);")
        trait_path = tmp_path / "traits.tsv"
        pd.DataFrame({"leaf_name": ["001", "002"], "group": ["x", "y"]}).to_csv(
            trait_path, sep="\t", index=False
        )
    ...
    def test_draw_accepts_na_literal_leaf_names_in_trait_table(self, tmp_nwk, tmp_path):
        ...
        trait_path = tmp_path / "traits.tsv"
        pd.DataFrame({"leaf_name": ["NA", "B"], "group": ["x", "y"]}).to_csv(
            trait_path, sep="\t", index=False
        )
    ...
    def test_draw_uses_explicit_panel_dimensions_and_root_marker(
    ...
    def test_draw_embeds_manifest_tip_images_and_expands_auto_height(

    ...
    for first_index, (first_owner, first_path) in enumerate(paths):
        for second_owner, second_path in paths[first_index + 1 :]:
            if first_owner is second_owner:
                continue
            for first_segment in zip(first_path, first_path[1:], strict=False):
                for second_segment in zip(second_path, second_path[1:], strict=False):
...
def test_randomized_complete_circular_tidy_layouts_remain_planar(seed):
    ...
    layout = make_tree_layout(
        ...
        terminal_extent_by_leaf={
            leaf: generator.uniform(0.0, 2.0) for leaf in tree.leaves()
        },
    )
...
def test_tip_track_legends_cover_constant_missing_and_many_categories():
    ...
    assert [label for label, _ in constant_legend] == ["score: 2", "score: missing"]
...
def test_spatial_root_stub_occupies_an_empty_angular_gap(layout_name):
    ...
    assert all(
        abs(math.sin(stub_angle - child_angle)) > 1e-6 for child_angle in child_angles
    )

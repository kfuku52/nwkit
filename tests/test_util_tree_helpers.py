...
class TestSpeciesGrouping:
    def test_monophyletic_duplicate_species_are_grouped(self):
        ...
        assert species_to_leaf_names["Homo_sapiens"] == [
            "Homo_sapiens_gene1",
            "Homo_sapiens_gene2",
        ]
...
class TestExtractSpeciesLabel:
    ...
    def test_taxonomic_keeps_species_label_qualifier(self):
        ...
        assert (
            extract_species_label("Dictyostelium_cf_discoideum", args=args)
            == "Dictyostelium_cf_discoideum"
        )
        assert (
            extract_species_label("Amoeba_sp_JDSRuffled", args=args)
            == "Amoeba_sp_JDSRuffled"
        )
        assert (
            extract_species_label(
                "Solanum_lycopersicum_cultivar_Heinz1706_gene1", args=args
            )
            == "Solanum_lycopersicum_cultivar_Heinz1706"
        )
        assert (
            extract_species_label("Escherichia_coli_serovar_O157_gene1", args=args)
    ...
    def test_taxonomic_taxonomy_query_falls_back(self):
        args = make_args(species_parser="taxonomic")
        assert (
            extract_taxonomy_query("Dictyostelium_cf_discoideum", args=args)
        ...
        assert (
            extract_taxonomy_query("Escherichia_coli_serovar_O157_gene1", args=args)
            == "Escherichia coli"
        )

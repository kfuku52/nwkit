...
class TestIntersectionMain:
    ...
    def test_tree_seq_commit_failure_restores_both_outputs(self, monkeypatch, tmp_path):
        ...
        assert out_seq.read_text() == "original sequences\n"
        leftovers = [
            path.name for path in tmp_path.iterdir() if path.name.startswith(".out.")
        ]
        assert leftovers == []
    ...
    def test_tree_seq_intersection_with_empty_seqout_writes_to_stdout(

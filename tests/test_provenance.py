...
def test_audit_path_cannot_modify_an_input_file(tmp_path):
    ...
    with pytest.raises(ValueError, match='distinct from input'):
        main([
            'label', '-i', str(infile), '-o', str(outfile),
            '--audit', str(infile),
        ])
...
def test_audit_path_cannot_create_a_declared_missing_input(tmp_path):
    ...
    with pytest.raises(ValueError, match='distinct from input'):
        main([
            'info',
            '-i', str(missing_infile),
            '--audit', str(missing_infile),
        ])
...
def test_audit_snapshots_input_before_in_place_output(tmp_path):
    ...
    record = json.loads(audit.read_text())
    input_record = next(item for item in record['inputs'] if item['path'] == str(tree_path.resolve()))
    output_record = next(item for item in record['outputs'] if item['path'] == str(tree_path.resolve()))
...
@pytest.mark.parametrize('derived_name', ['manifest.tsv', 'ATTRIBUTION.md', 'unmatched.tsv'])
def test_audit_rejects_image_derived_output_collisions(tmp_path, derived_name):
    ...
    with pytest.raises(ValueError, match='Output paths must be distinct'):
        main([
            ...
            '-i', str(infile),
            '--out-dir', str(out_dir),
            '--audit', str(out_dir / derived_name),
        ])

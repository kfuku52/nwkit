...
def test_property_map_renames_nhx_property(tmp_nwk, tmp_outfile):
    ...
    assert float(
        output.common_ancestor(["A", "B"]).props["confidence"]
    ) == pytest.approx(0.8)
...
def test_length_transfer_from_different_root_preserves_target_root_ratio(
    tmp_nwk, tmp_path
):
    target_path = tmp_nwk(
...
def test_root_edge_total_is_invariant_across_every_target_source_root_pair():
    ...
    for node in base.traverse():
        if not node.is_root:
            root_sides_by_split.setdefault(
                node_split(node), frozenset(node.leaf_names())
            )
    ...
    def rooted_on(descendant_taxa):
        ...
        candidates = [
            ...
            if not node.is_root and frozenset(node.leaf_names()) == descendant_taxa
        ]
        if not candidates:
            candidates = [
                node
...
def test_transfer_without_report_tracks_counts_without_building_rows():
    ...
    assert {leaf.name: float(leaf.dist) for leaf in result["tree"].leaves()} == {
        "A": 2.0,
        "B": 3.0,
        "C": 4.0,
    }

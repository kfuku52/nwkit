...
class TestDownloadMedia:
    ...
    def test_download_media_uses_content_type_to_resolve_extension(self, tmp_path):
        ...
        result = download_media(
            JPEGSession(), "https://example.org/download", str(destination)
        )
        ...
        class FailingSession:
            def get(self, *args, **kwargs):
                raise AssertionError(
                    "network should not be used when cache variant exists"
                )
    ...
    def test_download_media_rejects_html_returned_from_image_url(self, tmp_path):
        ...
        with pytest.raises(MediaDownloadError, match="non-image Content-Type"):
            download_media(
                HTMLSession(), "https://example.org/error.jpg", str(destination)
            )
    ...
    def test_download_media_enforces_content_length_limit(self, tmp_path):
        ...
        class LargeResponse:
            ...
            def iter_content(self, chunk_size=65536):
                raise AssertionError(
                    "oversized response should be rejected before streaming"
                )
...
class TestImagePostprocessing:
    ...
    def test_postprocess_media_file_trim_shape_square_center_crops_trimmed_result(
        self, tmp_path
    ):
        Image = pytest.importorskip("PIL.Image")
    ...
    def test_postprocess_media_file_trims_semantic_largest_rgb_component(
        self, tmp_path
    ):
        Image = pytest.importorskip("PIL.Image")
    ...
    def test_postprocess_media_file_trims_semantic_largest_alpha_component(
        self, tmp_path
    ):
        Image = pytest.importorskip("PIL.Image")
    ...
    def test_postprocess_media_file_trim_shape_square_without_trim_center_crops_full_image(
        self, tmp_path
    ):
        Image = pytest.importorskip("PIL.Image")
    ...
    def test_postprocess_media_file_rasterizes_svg_when_requested(
        ...
        monkeypatch.setattr(
            ...
            lambda source_path, max_edge=None: Image.new(
                "RGBA", (20, 10), (0, 0, 0, 255)
            ),
        )
    ...
    def test_postprocess_media_file_requires_cairosvg_for_svg_conversion(
        self, monkeypatch, tmp_path
    ):
        source = tmp_path / "shape.svg"
        source.write_text(
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                RuntimeError(
                    'SVG image post-processing requires the optional CairoSVG dependency. Install optional image-processing dependencies with: pip install "nwkit[image]"'
                )
            ),
        )
    ...
    def test_image_main_skips_optional_processing_deps_when_not_requested(
        self, monkeypatch, tmp_path
    ):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                AssertionError(
                    "Pillow should not be loaded without post-processing options"
                )
            ),
        )
...
class TestImageSecurityLimits:
    ...
    def test_cross_origin_redirect_drops_session_and_request_credentials(
        ...
        def fake_get(pinned_session, url, **kwargs):
            effective_headers = {
                str(key).lower(): value for key, value in pinned_session.headers.items()
            }
    ...
    def test_pinned_redirect_merges_response_cookies_into_caller_session(
        ...
        def fake_get(pinned_session, url, **kwargs):
            cookies_seen.append(
                pinned_session.cookies.get_dict(
    ...
    def test_ncbi_cache_replaces_corrupt_images_table(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            image_module, "_fetch_ncbi_archive_md5", lambda session: None
        )
    ...
    def test_ncbi_database_validation_handles_uri_characters_and_empty_tables(
        ...
        connection.execute(
            "CREATE TABLE images (taxid INTEGER, record_id TEXT, image_url TEXT)"
        )
        connection.commit()
        connection.close()

        assert not image_module._ncbi_images_database_is_valid(str(database_path))
    ...
    def test_ncbi_database_reloads_table_after_late_corrupt_row(
        ...
        def fake_ensure_table(args, session):
            ...
            payload = (
                valid_row + "corrupt later row\n" if calls["count"] == 1 else valid_row
            )

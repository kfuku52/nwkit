...
class TestMidpointRooting:
    ...
    def test_symmetric_clock_exact_distances(self):
        ...
        tree = TreeNode(newick='((A:2,B:2):1,(C:2,D:2):1);', format=1)
        rooted = midpoint_rooting(tree)
        dists = [rooted.get_distance(l) for l in rooted.get_leaves()]
    ...
    def test_pairwise_distance_preservation(self):
        ...
        rooted = midpoint_rooting(tree)
        for l1 in rooted.get_leaves():
            for l2 in rooted.get_leaves():
                if l1.name != l2.name:
...
class TestMadRooting:
    ...
    def test_wiki_exact_root_split(self):
        ...
        """
        tree = TreeNode(newick='((A:1,B:2):1,(C:3,(D:1,E:2):1):1);', format=1)
        original = TreeNode(newick='((A:1,B:2):1,(C:3,(D:1,E:2):1):1);', format=1)
        rooted = mad_rooting(tree)
...
class TestMvRooting:
    ...
    def test_clock_like_tree(self):
        ...
        tree = TreeNode(newick='((A:2,B:2):1,(C:2,D:2):1);', format=1)
        rooted = mv_rooting(tree)
        dists = [rooted.get_distance(leaf) for leaf in rooted.get_leaves()]
    ...
    def test_wiki_exact_root_position(self):
        ...
        tree = TreeNode(newick='((A:1,B:2):1,(C:3,(D:1,E:2):1):1);', format=1)
        rooted = mv_rooting(tree)
        dists = {l.name: rooted.get_distance(l) for l in rooted.get_leaves()}
    ...
    def test_three_taxa_exact(self):
        ...
        tree = TreeNode(newick='(A:1,B:3,C:5);', format=1)
        rooted = mv_rooting(tree)
        for l1 in rooted.get_leaves():
            for l2 in rooted.get_leaves():
                if l1.name != l2.name:
...
class TestTransferRoot:
    ...
    def test_pairwise_distance_preservation(self):
        ...
        tree_from = TreeNode(newick='((A:1,B:1):1,(C:1,D:1):1);', format=1)
        tree_to = TreeNode(newick='(A:2,B:3,(C:4,D:5):1);', format=1)
        original = TreeNode(newick='(A:2,B:3,(C:4,D:5):1);', format=1)
        result = transfer_root(tree_to, tree_from)
        for l1 in result.get_leaves():
            for l2 in result.get_leaves():
                if l1.name != l2.name:
                    orig_d = original.get_distance(l1.name, l2.name)
...
class TestRootMain:
    ...
    def test_wiki_midpoint_rooting(self, tmp_nwk, tmp_outfile):
        ...
        dists = {l.name: tree.get_distance(l) for l in tree.get_leaves()}
    ...
    def test_wiki_transfer_exact_distances(self, tmp_nwk, tmp_outfile):
        ...
        input1: (((a:1,b:1):1,c:1):1,((d:1,e:1),f:1):1):0;
        input2: ((((a:1,b:1):1,c:1),f:1):1,(d:3,e:3):1):0;
        Output: ((d:1,e:1):0.5,(f:1,(c:1,(b:1,a:1):1):2):0.5)Root:0;
        """
        path1 = tmp_nwk('(((a:1,b:1):1,c:1):1,((d:1,e:1),f:1):1):0;', 'tree1.nwk')
        path2 = tmp_nwk('((((a:1,b:1):1,c:1),f:1):1,(d:3,e:3):1):0;', 'tree2.nwk')

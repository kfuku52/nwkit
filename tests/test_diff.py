...
def test_unrooted_fail_ignores_arbitrary_newick_root_position(
        tmp_nwk, tmp_path):
    target_path = tmp_nwk(
...
def test_fail_on_difference_includes_matched_node_values(
        tmp_nwk, tmp_path, target_tree, source_tree, format, properties,
        target_class):
...
def test_fail_on_difference_uses_numeric_tolerance(tmp_nwk, tmp_path):
    ...
    source_path = tmp_nwk(
        '((A:1.0000000000005,B:1)90.00000000001:2.0000000000005,C:1)'
        ':1.0000000000005;',
        'source.nwk',
    )
    args = _diff_args(
...
def test_unrooted_fail_sums_root_edge_halves_across_rerooting(
        tmp_nwk, tmp_path):
    target_path = tmp_nwk(
...
def test_unrooted_fail_detects_physical_root_edge_length_difference(
        tmp_nwk, tmp_path):
    target_path = tmp_nwk(
        '((A:1,B:1):2,(C:1,(D:1,E:1):7):3);',
        'target.nwk',
    ...
    rows = pd.read_csv(outfile, sep='\t')
    root_edge = rows.loc[
        rows['shared_taxa_or_split'] == 'A,B|C,D,E'
    ].iloc[0]
...
def test_unrooted_root_edge_retains_unique_or_equal_annotations():
    ...
    edge_row = next(
        row for row in rows if row['comparison'] == 'unrooted_split'
    )

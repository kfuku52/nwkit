...
class TestConsensusMain:
    ...
    def test_singleton_clades_are_counted_once_and_lengths_are_collapsed(self, tmp_path):
        infile = _write_tree_collection(
            tmp_path,
            [
    ...
    def test_consensus_accepts_its_own_root_polytomy_output(self, tmp_path):
        ...
        tree = read_tree(second_output, format='auto', quoted_node_names=True, quiet=True)
        assert {frozenset(child.leaf_names()) for child in tree.get_children()} == {
            frozenset({'A'}),
            frozenset({'B'}),
            frozenset({'C'}),

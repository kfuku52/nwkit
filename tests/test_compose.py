...
def test_compose_combines_root_names_support_lengths_and_property(tmp_nwk, tmp_path):
    ...
    assert set(rows["source_property"]) >= {
        "root",
        "name",
        "support",
        "length",
...
def test_compose_preserves_support_and_lengths_across_different_rootings(
    ...
    root_length_by_side = {
        frozenset(child.leaf_names()): child.dist for child in output.get_children()
    }
    ...
    rows = pd.read_csv(report, sep="\t")
    abc_split = rows[
        (rows["source_property"] == "support") & (rows["target_taxa"] == "D,E,F")
    ].iloc[0]
...
def test_compose_all_component_sources_with_distinct_root_positions(tmp_nwk, tmp_path):
    ...
    name_source = tmp_nwk(
        "(C:1,((A:1,B:1)NAME_AB:1,(D:1,(E:1,F:1)NAME_EF:1)NAME_ABC:1):1)NAME_ROOT;",
        "names.nwk",
    )
    support_source = tmp_nwk(
...
def test_compose_split_resolves_equal_support_on_root_edges(tmp_nwk, tmp_path):
    ...
    root_edge_rows = rows[
        ...
        & (
            rows["ambiguity_resolution"].isin(
                ["unique_edge_value", "equal_edge_values"]
            )
        )
    ]
...
def test_compose_split_auto_uses_matching_side_for_conflicting_support(
    ...
    support_source = tmp_nwk(
        "((E:1,F:1)40:0.5,(D:1,(C:1,(A:1,B:1)20:1)30:1)50:0.5);",
        "support.nwk",
    )
    report = tmp_path / "composition.tsv"
    args = make_args(
    ...
    rows = pd.read_csv(report, sep="\t")
    ef_edge = rows[
        (rows["source_property"] == "support") & (rows["target_taxa"] == "E,F")
    ].iloc[0]
...
def test_compose_manifest_configures_root_edge_policy(tmp_path):
    ...
    supports.write_text("((E:1,F:1)40:0.5,(D:1,(C:1,(A:1,B:1)20:1)30:1)50:0.5);")
    manifest.write_text(
...
def test_compose_manifest_property_can_override_root_edge_policy(tmp_path):
    ...
    ef_row = rows[
        (rows["target_property"] == "edge_state") & (rows["target_taxa"] == "E,F")
    ].iloc[0]
...
def test_compose_failed_root_transfer_leaves_target_root_unchanged(tmp_nwk, tmp_path):
    ...
    output = read_tree(args.outfile, format="1", quoted_node_names=True, quiet=True)
    assert {frozenset(child.leaf_names()) for child in output.get_children()} == {
        frozenset({"A", "B", "C"}),
        frozenset({"D"}),
    }
    rows = pd.read_csv(report, sep="\t")
    assert rows.loc[rows["source_property"] == "root", "status"].tolist() == [
        "unmatched"
    ]

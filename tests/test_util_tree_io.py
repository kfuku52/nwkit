...
class TestReadTree:
    ...
    def test_read_tree_auto_warns_on_ambiguous_numeric_internal_labels(
        self, tmp_nwk, capsys
    ):
        path = tmp_nwk("((A:1,B:1)42:1,C:1)99:1;")
    ...
    def test_read_tree_preserves_subunit_support_values_when_mixed_with_percent_support(
        self, tmp_nwk
    ):
    ...
    def test_read_tree_auto_strict_raises_on_ambiguous_numeric_internal_labels(
        self, tmp_nwk
    ):
        path = tmp_nwk("((A:1,B:1)42:1,C:1)99:1;")
...
class TestWriteTree:
    ...
    def test_write_auto_no_subcommand_in_argv(self, tmp_nwk, tmp_outfile, monkeypatch):
        ...
        tree2 = read_tree(
            tmp_outfile, format="auto", quoted_node_names=True, quiet=True
        )
        assert set(tree2.leaf_names()) == {"A", "B", "C", "D"}
    ...
    def test_write_auto_respects_explicit_input_format(
        self, tmp_nwk, tmp_outfile, monkeypatch
    ):
        path = tmp_nwk("((A:1,B:1)AB:1,(C:1,D:1)CD:1)ROOT;")
        tree = read_tree(path, format="1", quoted_node_names=True, quiet=True)
        args = make_args(outfile=tmp_outfile)
    ...
    def test_write_auto_uses_tree_specific_format_after_multiple_reads(
        self, tmp_nwk, tmp_outfile, monkeypatch
    ):
    ...
    def test_write_preserves_numeric_internal_names_with_quotes(
    ...
    def test_write_restores_tree_when_name_serialization_fails(
        ...
        original_values = [(node, node.name, node.support) for node in tree.traverse()]
        ...
        assert calls == 3
        assert [
            (node, node.name, node.support) for node in tree.traverse()
        ] == original_values
        assert not os.path.exists(tmp_outfile)

    @pytest.mark.parametrize(
        "attribute,value,error_match",
        [
    ...
    def test_write_rejects_non_finite_tree_values_before_opening_output(
        ...
        target_node = (
            tree["A"] if attribute == "dist" else tree.common_ancestor(["A", "B"])
        )

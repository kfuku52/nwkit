...
def test_console_handler_error_is_concise_without_traceback(tmp_path):
    project_root = Path(__file__).parents[1]
    infile = tmp_path / 'broken.nwk'
    infile.write_text('(A:1,B:1)')
    result = subprocess.run(
        [sys.executable, '-m', 'nwkit', 'info', '-i', str(infile)],
        cwd=project_root,
        capture_output=True,
        check=False,
        text=True,
    ...
    assert 'nwkit: error:' in result.stderr
    assert 'Traceback' not in result.stderr


def test_console_debug_option_preserves_traceback(tmp_path):
    project_root = Path(__file__).parents[1]
    infile = tmp_path / 'broken.nwk'
    infile.write_text('(A:1,B:1)')
    result = subprocess.run(

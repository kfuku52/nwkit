...
| `contrast --trait` with `--biological-id` | `leaf_name`, biological-ID column, and selected numeric traits | Multiple rows per tip are biological observations. Optional technical IDs are nested within leaf and biological ID; optional batch values are categorical and complete. Missing trait cells are allowed, but every tip needs an estimable mean. |
